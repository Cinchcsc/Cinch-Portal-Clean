...
if (!configured) console.warn('[supabaseAdmin] missing SUPABASE_URL/NEXT_PUBLIC_SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY');
...
// Historical rebuilds and payload rewrites can legitimately hold a single PostgREST request open
// well past 2 minutes under load. We observed targeted historical-repair runs still alive and
// retryable at the app layer while this client-side 120s abort was firing first, which means the
// client was killing viable work before the route/runtime budget had actually expired. Keep a ceiling
// so genuinely wedged requests still release, but align it with the 300s serverless budget and leave
// enough headroom for JS cleanup/logging after a slow-but-valid DB round trip.
const SUPABASE_FETCH_TIMEOUT_MS = 270_000;

const noStoreFetch = async (input, init = {}) => {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(new Error(`Supabase fetch timed out after ${SUPABASE_FETCH_TIMEOUT_MS}ms`)), SUPABASE_FETCH_TIMEOUT_MS);
