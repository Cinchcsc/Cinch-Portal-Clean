...
export async function runCockpitPull({ concurrency = Number(process.env.SITELINK_PULL_CONCURRENCY) || 1, triggerLabel = null, skipLockCheck = false } = {}) {
  ...
  if (!skipLockCheck) {
    ...
    if (!unlocked.ok) {
      ...
      else await recordCompletedPullLog('cockpit', 'skipped', unlocked.message, startedAtIso);
  ...
  const finishOrBackfillLog = async (status, detail) => {
    ...
    return recordCompletedPullLog('cockpit', status, detail, startedAtIso);
  };
  if (!skipLockCheck) {
  ...
  if (!locations.length) { await finishOrBackfillLog('error', 'SITELINK_LOCATIONS not set'); throw new Error('SITELINK_LOCATIONS not set'); }
