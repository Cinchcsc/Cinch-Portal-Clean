...
export const REPORTS = {
  ...
  lead_funnel: { method: 'InquiryTracking', dated: true, parse: (rows, startDate, endDate) => {
    ...
    // reservationContactHashes: ADDED 6 Jul 2026 (as reservationEmailHashes) for the Enquiry ->
    ...
    // EXTENDED 17 Jul 2026 (task #301): this pool now mixes in phone hashes from reservation-stage rows
    // too (a flat array is fine on this side — we only need "is this hash present anywhere", not which
    // row it came from), so an enquiry can match on EITHER contact method.
    const reservationContactHashes = [];
    ...
    for (const r of rows) {
      if (isReservationStage(r)) {
        ...
        const eh = emailHash(r.sEmail); if (eh) reservationContactHashes.push(eh);
        const ph = phoneHash(r.sPhone); if (ph) reservationContactHashes.push(ph);
      }
      ...
      if (yes(r.iReservationConvertedToLease) || /reserv/i.test(str(r.sCallType))) res++;   // kept for back-compat only — confirmed unreliable, no longer used (see reservationContactHashes above)
      ...
      // checked against "reservation contacts this period" (reservationContactHashes, built from the
      ...
      // reservationConversions tooltip for the honest "lower bound" framing shown on the widget — now
      // measuring the right thing instead of a tautology, plus phone as a second key (17 Jul 2026).
      if (!isReservationStage(r)) {
        const eh = emailHash(r.sEmail);
    ...
    return {
      ...
      reservation_contact_hashes: reservationContactHashes,
