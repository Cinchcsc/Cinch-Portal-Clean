...
// phoneHash — ADDED 17 Jul 2026 (task #301, Michael: "add phone and walk ins both as matching keys"
// for the Enquiry -> Reservation widget). Checked live data before building this: the "phone/walk-in
// leads rarely have a usable email" assumption behind the original 6 Jul fix does NOT hold for this
// account — portfolio-wide July 2026 InquiryTracking data shows sEmail populated on 90%+ of rows on
// EVERY channel, phone included. So this isn't primarily about covering an email gap; it's a second,
// independent identifier that catches genuine conversions where the same person's email has a typo or
// differs between their enquiry-stage row and their later reservation-stage row, but their phone
// number stays the same (and vice versa — some rows have a phone typo but a clean email).
// Normalizes the UK format variants actually seen in the raw sPhone column (07984202066 / 447984202066
// / 07984 202 066 / 00447984202066 — with-and-without leading 0, with-and-without 44/0044 country
// prefix, with-and-without spaces) by stripping all non-digits and keeping only the last 10, which is
// the UK "national significant number" shared by all of those variants. Fewer than 10 digits left is
// treated as unusable/junk (returns null) rather than risk a false match on a truncated number.
// Same PII-safety rule as emailHash: hashed immediately, raw digits never stored.
const phoneHash = (v) => {
...
export const REPORTS = {
  ...
  rent_roll: { method: 'RentRoll', dated: true, parse: (rows) => {
    ...
    for (const r of rows) {
      ...
      unitRows.push({ unit: str(r.sUnit), type: t, area: areaR, groupKey: `${t}|${areaR}`, stdRate: R2(stdRent), rent: R2(rent), leaseDays });
    }
  ...
  lead_funnel: { method: 'InquiryTracking', dated: true, parse: (rows, startDate, endDate) => {
    ...
    // EXTENDED 17 Jul 2026 (task #301): each qualifying enquiry row now carries BOTH its email hash
    // and its phone hash (phoneHash, see top of file) instead of email alone — see inquiryContacts
    // below. Restructured from a flat inquiryEmailHashes array to one {emailHash,phoneHash} object per
    // row so a person who matches on BOTH keys still only counts once (checked in buildPayload.js).
    const inquiryTenantIds = [];
    const inquiryContacts = [];
    // reservationContacts: ADDED 6 Jul 2026 (as reservationEmailHashes, a flat array) for the Enquiry ->
    // Reservation fix — confirmed via probe:enquiry-reservation that BOTH previous candidates are
    // dead: `iReservationConvertedToLease` is barely populated (4.0%, same broken-flag class as
    // iInquiryConvertedToLease), and WaitingID/TenantID is not even stable across a single lead's OWN
    // stage progression within this same report (0 overlap between 1,341 Inquiry-stage IDs and 369
    // Reservation-stage IDs — each stage transition apparently gets a fresh ID). Same fix as Enquiry ->
    // Move-In: email is a real-world identifier stable across a person's journey regardless of
    // internal ID churn. Hashed (SHA-256) immediately, same PII-safety rule as inquiryContacts above.
    // EXTENDED 17 Jul 2026 (task #301): started mixing in phone hashes from reservation-stage rows too.
    // RESTRUCTURED 17 Jul 2026 (task #303, Michael: "if they enquired last month, but moved in this
    // month, it should carry over to this month") — buildPayload.js's reservationConversions now
    // attributes a conversion to the RESERVATION's month, looking back at enquiries from that month OR
    // the one before (instead of attributing to the ENQUIRY's month and looking forward, which can
    // never work for the live in-progress month since next month's data doesn't exist yet). That means
    // this pool now needs to be walked ONE ROW AT A TIME on the reservation side too (so a reservation
    // matching on both email AND phone still only counts once) — changed from a flat array of
    // individual hash strings to one {emailHash,phoneHash} object per reservation-stage row, same shape
    // as inquiryContacts.
    ...
    for (const r of rows) {
      if (isReservationStage(r)) {
        reservationStageCount++;
        const eh = emailHash(r.sEmail);
        const ph = phoneHash(r.sPhone);
        if (eh || ph) reservationContacts.push({ emailHash: eh, phoneHash: ph });
      }
      ...
      if (yes(r.iReservationConvertedToLease) || /reserv/i.test(str(r.sCallType))) res++;   // kept for back-compat only — confirmed unreliable, no longer used (see reservationContacts above)
      if (yes(r.bCommercial)) biz++;
      if (r.TenantID != null) inquiryTenantIds.push(String(r.TenantID));
      // FIXED 16 Jul 2026 (deep re-audit, self-match bug): only push to inquiryContacts if this row is
      // NOT itself a reservation-stage event — otherwise a reservation created this period gets
      // checked against "reservation contacts this period" (reservationContacts, built from the
      // same rows just above) and trivially matches itself, which isn't a real enquiry -> reservation
      // conversion at all. Confirmed via probe-enquiry-reservation-selfmatch.js: 350 of 423 portfolio
      // "conversions" (83%) were exactly this self-match, inflating the live rate to 19.6% vs legacy's
      // 12.5%. Excluding them drops it to 3.1% (email-only) / see buildPayload.js's
      // reservationConversions tooltip for the honest framing shown on the widget — now measuring the
      // right thing instead of a tautology, plus phone as a second key (17 Jul 2026).
      if (!isReservationStage(r)) {
        const eh = emailHash(r.sEmail);
        const ph = phoneHash(r.sPhone);
        if (eh || ph) inquiryContacts.push({ emailHash: eh, phoneHash: ph });
      }
    }
    return {
      ...
      inquiry_contacts: inquiryContacts,
      reservation_contacts: reservationContacts,
  ...
  rate_changes: { method: 'TenantRentChangeHistory', dated: true, parse: (rows, startDate, endDate) => {
    ...
    for (const r of rows) {
      if (!inWindow(r.dLRateFrom)) continue;
  ...
  // live probes — NOT a summary, a genuine per-tenant/per-charge report). "DiscountSummary" and
  // "UnitStatus" (SiteLink's own web UI report picker) are NOT callable API methods — confirmed
  // against the live WSDL (only UnitPriceList/Discounts/UnitPayments matched a discount/unit search).
  ...
  discounts: { method: 'Discounts', dated: true, parse: (rows, startDate, endDate) => {
    const byPlan = {};
    for (const r of rows) {
      const plan = str(r.sConcessionPlan) || '(unspecified)';
      const p = (byPlan[plan] ??= { units: new Set(), discountSum: 0 });
      p.units.add(str(r.sUnitName));
