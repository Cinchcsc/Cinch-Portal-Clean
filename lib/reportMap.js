...
export const REPORTS = {
  ...
  occupancy: { method: 'OccupancyStatistics', dated: true, parse: (rows) => {
    ...
    for (const r of rows) {
      ...
      // column." OccupancyStatistics' own raw rows weren't being checked for this field at all before
      // today — only RentalActivity (a sibling report in the same reporting family) was confirmed to
      // expose it directly (OccupiedArea/TotalArea/VacantArea/NetArea, see rental_activity below).
      // Falls back to the previous Area × Occupied derivation if the raw field isn't present on a
      // given pull — same numeric result either way when both are available, since a row's
      // OccupiedArea should equal its own Area × Occupied by construction, so this is a safe change:
      // it can only make the figure more direct/accurate, never break it.
  ...
  lead_funnel: { method: 'InquiryTracking', dated: true, parse: (rows, startDate, endDate) => {
    ...
    for (const r of rows) {
      ...
      const eh = emailHash(r.sEmail);
      // FIXED 16 Jul 2026 (deep re-audit, self-match bug): only push to inquiryEmailHashes if this
      // row is NOT itself a reservation-stage event — otherwise a reservation created this period
      // gets checked against "reservation emails this period" (reservationEmailHashes, built from
      // the same rows just above) and trivially matches itself, which isn't a real enquiry ->
      // reservation conversion at all. Confirmed via probe-enquiry-reservation-selfmatch.js: 350 of
      // 423 portfolio "conversions" (83%) were exactly this self-match, inflating the live rate to
      // 19.6% vs legacy's 12.5%. Excluding them drops it to 3.1% — very likely still an undercount
      // (phone/walk-in leads rarely have a usable email on file to match on at all), but it's now
      // measuring the right thing instead of a tautology. See buildPayload.js's reservationConversions
      // tooltip for the honest "lower bound" framing shown on the widget.
      if (eh && !isReservationStage(r)) inquiryEmailHashes.push(eh);
    }
    return {
