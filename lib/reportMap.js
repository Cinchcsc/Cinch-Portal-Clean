...
export const REPORTS = {
  ...
  true_revenue: { customReportId: 781861, dated: true, parse: (rows) => {
    ...
    // FIXED 8 Jul 2026: 'Tax1AdjustmentsThisPeriod' was the wrong word order -- the actual raw
    // SiteLink column (confirmed via scripts/dump-truerevenue-tables.js live sample dump for L008) is
    // 'ThisPeriodTax1Adjustments'. The old name never matched, so num() silently fell through to 0 and
    // 'taxAdj' (the True Revenue table's "Tax Adj" display column) always read 0. Does not affect Real
    // Rate -- that uses 'truePeriod'/'adj' (TruePeriod/ThisPeriodAdjustments), both already correctly
    // named here and unaffected by this fix.
    const cols = ['InvoicedThisPeriod', 'InvoicedTax1ThisPeriod', 'ThisPeriodTax1Adjustments', 'NetTax1ThisPeriod', 'DeferredRevenue', 'PriorPeriodDeferred', 'ThisPeriodAdjustments', 'PriorPeriodAdjustments', 'TruePeriod'];
