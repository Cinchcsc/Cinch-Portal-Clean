...
import { formatLocalYmd, startOfReportingToday } from './reportingPeriod.js';
...
export const REPORTS = {
  ...
  rent_roll: { method: 'RentRoll', dated: true, parse: (rows) => {
    ...
    const asOf = startOfReportingToday();
    let tenants = 0, autobill = 0, stayDays = 0, stayN = 0, stayRent = 0;
    ...
    for (const r of rows) {
      ...
      if (!isBlankDate(r.dLeaseDate)) {
        const d = (asOf - new Date(r.dLeaseDate)) / 86400000;
        if (d > 0) { stayDays += d; stayN++; stayRent += rent; leaseDays = R(d); }
      }
    ...
    return {
      ...
      stay_days_sum: R2(stayDays),
      stay_count: stayN,
      stay_rent_sum: R2(stayRent),
  ...
  management: { method: 'ManagementSummary', dated: true, parse: (rows, startDate, endDate, raw) => {
    ...
    const delinquentAgeing = { '0-10': 0, '11-30': 0, '31-60': 0, '61-90': 0, '91-120': 0, '121-180': 0, '181-360': 0, '361+': 0 };
    for (const r of extractNamedTable(raw, 'Unpaid')) {
      const rawBucket = str(r.Period);
      const bucket = rawBucket === '>360' ? '361+' : rawBucket;
      if (!Object.prototype.hasOwnProperty.call(delinquentAgeing, bucket)) continue;
      const bucketTotal = num(r, 'dcDlqntTot');
      delinquentAgeing[bucket] += bucketTotal;
      if (bucket === '0-10' || bucket === '11-30') continue;   // "delinquent" = 30+ days (R6's own rule)
      delinquent30Total += bucketTotal; delinquent30Units += num(r, 'iDelUnits');
    }
    const roundedDelinquentAgeing = {};
    for (const [bucket, total] of Object.entries(delinquentAgeing)) roundedDelinquentAgeing[bucket] = Math.round(total);
    const roundedDelinquent30Total = ['31-60', '61-90', '91-120', '121-180', '181-360', '361+']
      .reduce((sum, bucket) => sum + (roundedDelinquentAgeing[bucket] || 0), 0);
    ...
    return {
      ...
      delinquent_30plus_total: roundedDelinquent30Total, delinquent_30plus_units: delinquent30Units,
      delinquent_ageing: roundedDelinquentAgeing,
  ...
  scheduled_outs: { method: 'ScheduledMoveOuts', dated: true, parse: (rows) => {
    ...
    const asOfDay = formatLocalYmd(startOfReportingToday());
    let upcoming = 0, stale = 0;
    for (const r of rows) {
      const day = isBlank(r.dSchedOut) ? null : sourceDayKey(r.dSchedOut);
      if (day && day >= asOfDay) upcoming++; else stale++;
    }
  ...
  insurance_roll: { method: 'InsuranceRoll', dated: true, parse: (rows, startDate, endDate) => {
    ...
    // FIX 27 Jul 2026 (task #426, confirmed via `npm run probe:insurance-roll-iactive-stability`: the
    // SAME closed month, June 2026, read 916 new customers on one run and 904 a few days later) — the
    // 6 Jul design above also gated insured_new_customers by iActive, same structural bug as the
    // now-fixed task #422 Snapshot bug: dMovedIn is a fixed historical fact, but iActive is "is this
    // policy active TODAY" — a customer who moved in with insurance in June and later cancelled in
    // July silently vanished from June's already-closed "new customers" count on any later re-pull/
    // reparse. iActive stays the right gate for insured/premium/coverage (an intentional "as of now"
    // penetration snapshot, correctly capped at ≤100%) — it's wrong only for the dMovedIn-windowed
    // new-customer flow metric below, which must depend on the date window alone, not current status.
    const R2i = (n) => Math.round((n + Number.EPSILON) * 100) / 100;
    let newCount = 0, newPremium = 0, newCoverage = 0;
    for (const r of rows) {
      if (yes(r.iActive)) { insured++; premium += num(r, 'dcPremium'); coverage += num(r, 'dcCoverage'); }   // current-book penetration ≤ 100%, "as of now" by design
      if (inSourceDayWindow(r.dMovedIn, startDate, endDate)) { newCount++; newPremium += num(r, 'dcPremium'); newCoverage += num(r, 'dcCoverage'); }   // historical fact — independent of current iActive status
    }
    return {
  ...
  // FIXED 27 Jul 2026 (found via probe-stale-parse-shape-sweep.js: new_policies read a flat 0 for
  // nearly every site this month) — npm run probe:insurance-activity-newpolicy-values confirmed
  // sNewPolicy/sCancelledPolicy don't use the Y/N or true/false convention yes() expects: SiteLink
  // marks these two columns with the literal string "X" when true, and an attribute-only empty
  // element (flattens to {attributes:{...}}, no `_` text) when false. yes() is correct for every
  // other Y/N-string field in this file, but never matched "X", so sNewPolicy silently read false on
  // every row — explaining both new_policies=0 and new_premium=0. sCancelledPolicy has the identical
  // "X" convention and was equally broken, but `cancellations` still looked plausible only because of
  // the `|| yes(r.bCancelled)` fallback beside it (bCancelled is a real "true"/"false" string, which
  // yes() does match) — confirmed via the probe that cancellations came out identical to the
  // bCancelled-only count, i.e. sCancelledPolicy's "X" rows were contributing nothing.
  insurance_activity: { method: 'InsuranceActivity', dated: true, parse: (rows, startDate, endDate, raw) => {
    ...
    const markedX = (v) => String(v ?? '').trim().toUpperCase() === 'X';   // SiteLink's checkbox-style flag for these 2 columns specifically
    for (const r of activityRows) {
      if (markedX(r.sNewPolicy) || yes(r.sNewPolicy)) { newPol++; newPrem += num(r, 'dcPremium'); }
      if (markedX(r.sCancelledPolicy) || yes(r.sCancelledPolicy) || yes(r.bCancelled)) cancelled++;
    }
  ...
  lead_funnel: { method: 'InquiryTracking', dated: true, parse: (rows, startDate, endDate, raw) => {
    ...
    const channels = {}; let phone = 0, walkin = 0, web = 0, email = 0, other = 0, conv = 0, biz = 0;
    ...
    for (const r of activityRows) {
      ...
      if (isReservationStage(r) && isVisibleMarketingChannel(r.sInquiryType)) {
        const convertedDay = sourceDayKey(r.dConverted_ToRsv);
        if (convertedDay && convertedDay >= ymd(startDate) && convertedDay <= ymd(endDate)) reservationMadeVisible++;
      }
      ...
      if (yes(r.iInquiryConvertedToLease)) conv++;
      // Marketing page/channel conversion numerator — FIXED 24 Jul 2026 (post-refresh legacy
      // parity audit). iInquiryConvertedToLease alone materially understates the live legacy
      // Marketing page, especially for Phone/Walk-in. A portfolio-wide July 2026 probe against the
      // freshly rebuilt raw InquiryTracking rows showed legacy at 14.8% while the old
      // channels[label].converted logic landed 10.6%; counting a visible enquiry as converted when
      // EITHER SiteLink conversion flag is true (`iInquiryConvertedToLease` OR
      // `iReservationConvertedToLease`) lands at 14.7% overall and closely matches the legacy
      // aggregate channel percentages too (Phone 42.0 vs legacy 40.8, Walk-in 51.9 vs 49.8, Web
      // 10.1 vs 10.5). Keep top-level `conversions` on the narrower iInquiryConvertedToLease field
      // for the separate Enquiry -> Move-In metric, but widen the per-channel converted count that
      // Marketing / Enquiry -> Reservation actually reads.
      if (yes(r.iInquiryConvertedToLease) || yes(r.iReservationConvertedToLease)) channels[label].converted++;
      // REMOVED 24 Jul 2026 (task #428/#429, bug-class sweep after the Daily Snapshot fix): this used to
      // be its own `res` counter, incremented on `yes(r.iReservationConvertedToLease) ||
      // /reserv/i.test(str(r.sCallType))` and returned below as `reservations`. Its own comment claimed
      // "confirmed unreliable, unused (see reservation_stage_count)" — the "unreliable" part was right
      // (iReservationConvertedToLease is the exact same live/mutable flag family proven to swing 33->5
      // within a single afternoon for the Daily Snapshot bug, task #422), but "unused" was wrong: this
      // field is read by the custom widget builder's "Reservations (InquiryTracking)" column, a
      // reserved-sqft estimate, the legacy bootstrap API, and range-summed totals (see buildPayload.js/
      // page.js/app/api/bootstrap.js/route.js). Fix: just return reservationStageCount (below) instead —
      // it already counts the same underlying concept (enquiries placed in this window that reached
      // reservation stage) via the reliable pattern (isReservationStage + dPlaced window) already relied
      // on for the Reservations-vs-Move-outs widget and the Enquiry->Reservation numerator, so this is a
      // same-window, same-definition swap, not a new metric.
    ...
    return {
      ...
      conversions: conv, reservations: reservationStageCount, business_enquiries: biz, channels,
      reservation_stage_count: reservationStageCount,
      reservation_made_visible: reservationMadeVisible,
  ...
  reservations: { ccws: true, dated: false, parse: (rows) => {
    ...
    for (const r of rows) {
      ...
      const neededDay = isBlank(r.dNeeded) ? null : sourceDayKey(r.dNeeded);
      if (isCancelled) { cancelled++; continue; }
      if (!(neededDay && neededDay >= asOfDay)) continue;
  ...
  true_revenue: { customReportId: 781861, dated: true, parse: (rows, startDate, endDate, raw) => {
    ...
    // by_type_desc — ADDED 24 Jul 2026 (task #308 follow-up, Self Storage Real Rate: Michael, "self
    // storage is the same rate but you [need to] put a filter on the type to only be self storage
    // not everything"). by_desc/by_type above each collapse Table1 to ONE axis (every unit type's
    // Rent combined, or Self Storage's every charge type combined) — neither can answer "Self
    // Storage's Rent alone". Table1 is already one row per (ChargeDesc, UnitType) combination (see
    // comment above), so this just exposes that same raw grain on BOTH axes at once instead of
    // collapsing either one — no new SOAP call, same trRows. Grouped (not a raw 1:1 map) in case
    // SiteLink ever returns more than one row for the same combination, same defensive summing as
    // groupBy() above.
    const typeDesc = {};
    for (const r of trRows) {
      const k = (str(r.UnitType) || 'Other') + '|' + (str(r.ChargeDesc) || 'Other');
      const o = (typeDesc[k] ??= { type: str(r.UnitType) || 'Other', desc: str(r.ChargeDesc) || 'Other', truePeriod: 0 });
      o.truePeriod += num(r, 'TruePeriod');
    }
    for (const k of Object.keys(typeDesc)) typeDesc[k].truePeriod = R2v(typeDesc[k].truePeriod);
    return { by_desc: groupBy('ChargeDesc'), by_type: groupBy('UnitType'), by_type_desc: Object.values(typeDesc), period_days };
  }},
...
export async function pullReport(key, loc, startDate, endDate, opts = {}) {
  const r = REPORTS[key];
  ...
  const parseEndDate = opts.parseEndDate || endDate;
  ...
  return { data: r.parse(rows, startDate, parseEndDate, raw), rowcount: rows.length, raw };
}
