...
import { callReport, callReservationList, callCustomReport } from './sitelink.js';
...
export const REPORTS = {
  ...
  // 1 Jul 2026). No other file may recompute these; buildPayload.js reads the fields below as-is.
  //   Rate per Sq Ft (No Concessions)        = (Σ dcRent           ÷ Σ Area Rented) × 12
  //   Self Storage Rate per Sq Ft             = same, rows where sTypeName = "Self Storage" only
  //   Real Rate per Sq Ft (incl. concessions) = (Σ dcStandardRate  ÷ Σ Area Rented) × 12
  //   Self Storage Real Rate per Sq Ft        = same, rows where sTypeName = "Self Storage" only
  // (Spec calls the standard-rent field "dcStandardRate" — the actual SiteLink RentRoll column
  // confirmed against a live export is `dcStandardRate`; there is no `dcStandardRent` column.)
  ...
  rent_roll: { method: 'RentRoll', dated: true, parse: (rows) => {
    ...
    for (const r of rows) {
      ...
      const a = num(r, 'Area', 'Area1'), rent = num(r, 'dcRent'), stdRent = num(r, 'dcStandardRate'), t = str(r.sTypeName) || 'Other';
    ...
    return {
      ...
      rate_per_sqft_ann: rate(all.rent, all.area),              // Total Rate (No Concessions)
      real_rate_per_sqft_ann: rate(all.stdRent, all.area),      // Total Real Rate (incl. concessions)
      // raw sums, for portfolio/filter-level re-aggregation without averaging rates:
      rent_sum: R(all.rent), std_rent_sum: R(all.stdRent), area_sum: R(all.area),
      self_storage: {
        occupied_area: R(ss.area), monthly_rent: R(ss.rent),
        rate_per_sqft_ann: rate(ss.rent, ss.area),              // Self Storage Rate (No Concessions)
        real_rate_per_sqft_ann: rate(ss.stdRent, ss.area),      // Self Storage Real Rate
  ...
  management: { method: 'ManagementSummary', dated: true, parse: (rows) => {
    const m = {};
  ...
  // each time they progress a stage. Counting all rows (as this file did before) stacked three
  // different widgets' worth of events into one number — Reservation-stage rows belong to the
  // Reservations widget, Move-In-stage rows belong to Move-ins, and only "Inquiry"-stage rows are
  // genuine new enquiries. Filtering to sRentalType="Inquiry" took a real Bicester example from 192
  // rows (vs a legacy target of 122) down to 148 — still not exact, but the right kind of fix rather
  // than a formula tweak; the residual ~20% gap needs another data pass to close.
  ...
  //   webCombined = Web + Email                     (spec's displayed "Web" tile)
  //   total       = Phone + WalkIn + Web + Email     (sum of the four raw counts)
  // AMENDED 3 Jul 2026: sInquiryType's phone/walkin/web split is NO LONGER used for the portal's
  // Phone/Walk-in/Web tiles (buildPayload.js now sources those from ManagementSummary's own
  // phone_leads/web_leads/walkin_leads instead — confirmed a much closer match to the legacy
  // portal's ratio via probe:enquiries-mgmt-breakdown). This parser's phone/walkin/web/email counts
  // are kept for `channels`/webOnly/email drill-down use, and total_enquiries remains authoritative.
  lead_funnel: { method: 'InquiryTracking', dated: true, parse: (rows) => {
    const isInquiryStage = (r) => { const t = str(r.sRentalType).toLowerCase(); return !t || t === 'inquiry'; };   // blank sRentalType tolerated (older data may not have this column)
  ...
  true_revenue: { customReportId: 781861, dated: true, parse: (rows) => {
    ...
    const cols = ['InvoicedThisPeriod', 'InvoicedTax1ThisPeriod', 'Tax1AdjustmentsThisPeriod', 'NetTax1ThisPeriod', 'DeferredRevenue', 'PriorPeriodDeferred', 'ThisPeriodAdjustments', 'PriorPeriodAdjustments', 'TruePeriod'];
    const outKeys = ['invoiced', 'taxInvoiced', 'taxAdj', 'netTax', 'deferred', 'deferredPrev', 'adj', 'adjPrev', 'truePeriod'];
    ...
    const groupBy = (field) => {
      ...
      for (const r of rows) {
        ...
        const o = (g[k] ??= Object.fromEntries(outKeys.map((k2) => [k2, 0])));
        cols.forEach((c, i) => { o[outKeys[i]] += num(r, c); });
      }
      for (const k of Object.keys(g)) for (const k2 of outKeys) g[k][k2] = R2v(g[k][k2]);
...
export async function pullReport(key, loc, startDate, endDate) {
  ...
  // window filter (see REPORTS.insurance_roll above). Every other parser's signature is (rows) =>
  // {...} and simply ignores the extra args, so this is a safe, additive change.
  return { data: r.parse(rows, startDate, endDate), rowcount: rows.length };
}
