...
export const REPORTS = {
  ...
  move_ins_outs: { method: 'MoveInsAndMoveOuts', dated: true, parse: (rows) => {
    ...
    return { move_ins: mi, move_outs: mo, net_units: mi - mo, moved_in_area: R(inArea), moved_out_area: R(outArea), net_area: R(inArea - outArea), move_in_tenant_ids: moveInTenantIds, moved_in_rental_rate_sum: R2(inRateSum) };
  }},
  ...
  lead_funnel: { method: 'InquiryTracking', dated: true, parse: (rows, startDate, endDate) => {
    ...
    // digits to ~10%), but a close match for a plain COUNT RATIO instead (reservation-stage rows this
    // period ÷ enquiries this period = 23.9% raw against our own numbers, same ballpark as legacy's
    // 19.8% given our enquiry count already runs ~10% below legacy's, a known/accepted gap). So legacy
    // isn't tracking individual leads here at all — just dividing two independent aggregates, same as
    // the Enquiry -> Move-In tile already does. reservationStageCount below (unchanged, added 6 Jul
    // for the "Reservations vs Move-outs" widget) is now ALSO the numerator for Enquiry -> Reservation
    // — see buildPayload.js's reservationConversions.
    ...
    for (const r of rows) {
      if (isReservationStage(r)) reservationStageCount++;
