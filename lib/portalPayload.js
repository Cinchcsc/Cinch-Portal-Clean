...
export const CURRENT_MONTH_SLICE_VERSION = '2026-08-10-live-current-v3';
const LIVE_CURRENT_RESULT_REUSE_MS = 5000;
const LIVE_CURRENT_WARMUP_DELAY_MS = 5000;
const LAST_GOOD_CURRENT_PAYLOAD_REUSE_MS = 15 * 60 * 1000;
const LAST_GOOD_STORED_PAYLOAD_REUSE_MS = 15 * 60 * 1000;
const STORED_CURRENT_SLICE_WARMUP_DELAY_MS = 2500;
const STORED_PORTAL_READ_TIMEOUT_MS = 15000;
const STORED_PORTAL_CURRENT_SLICE_READ_TIMEOUT_MS = 8000;
const RAW_FRESHNESS_PROBE_TIMEOUT_MS = 10000;
const LIVE_REFRESH_LOCK_PROBE_TIMEOUT_MS = 2500;
...
let sharedLastGoodCurrentPayload = {
  monthKey: null,
  result: null,
  resultAtMs: 0,
};
let sharedLastGoodStoredPayload = {
  result: null,
  resultAtMs: 0,
};
...
function derivePrevMonthKey(currentMonth) {
...
function normalizeStoredPayload(payload) {
  ...
  return {
    ...payload,
    // Preserve the ACTUAL historical build marker from storage. Treating a missing legacy marker as
    // "already current" hides stale historical slices from every downstream repair gate.
...
function buildPersistedCurrentSlice(payload, generatedAt = null) {
  if (!payload || typeof payload !== 'object') return null;
  if (!payload.current_month || !Array.isArray(payload.sites) || !payload.totals) return null;
  const normalized = normalizeStoredPayload({
  ...
  if (!hasReadablePortalCurrentSlice(normalized)) return null;
  return {
    build_version: normalized.build_version || PORTAL_PAYLOAD_BUILD_VERSION,
    current_month_slice_version: normalized.current_month_slice_version || null,
    current_month: normalized.current_month || payload.current_month,
    sites: normalized.sites,
    totals: normalized.totals,
  };
}

export function buildPortalPayloadWriteRow(payload, generatedAt = new Date().toISOString()) {
  const currentSlice = buildPersistedCurrentSlice(payload, generatedAt);
  return {
    id: 1,
    generated_at: generatedAt,
    payload: normalizePortalPayloadForStorage(payload),
    current_month: currentSlice?.current_month || payload?.current_month || null,
    current_slice: currentSlice ? normalizePortalPayloadForStorage(currentSlice) : null,
    build_version: payload?.build_version || PORTAL_PAYLOAD_BUILD_VERSION,
    current_month_slice_version: currentSlice?.current_month_slice_version || payload?.current_month_slice_version || null,
  };
}

export function decodePortalPayloadStorageValue(payload) {
...
function isPortalPayloadSidecarSchemaError(message) {
  return /column .* does not exist|schema cache/i.test(String(message || ''))
    && /current_slice|current_month_slice_version|build_version|current_month/.test(String(message || ''));
}

async function fetchPortalCurrentSliceRow() {
  try {
    return await retryOnStatementTimeout(async () => {
      const { data, error } = await admin
        .from('portal_payload')
      ...
      if (error) throw new Error(error.message);
      return data?.current_slice ? data : null;
    });
  } catch (error) {
    if (isPortalPayloadSidecarSchemaError(error?.message || error)) return null;
    throw error;
  }
}
...
export async function readPortalPayload() {
  ...
  const rawPayload = decodePortalPayloadStorageValue(data.payload);
  const payload = {
...
async function readPortalPayloadCurrentSlice() {
  const data = await fetchPortalCurrentSliceRow();
  if (!data?.current_slice) return null;

  const rawPayload = decodePortalPayloadStorageValue(data.current_slice);
  const payload = normalizeStoredPayload({
    ...rawPayload,
    build_version: data.build_version || rawPayload?.build_version || PORTAL_PAYLOAD_BUILD_VERSION,
    current_month_slice_version: data.current_month_slice_version || rawPayload?.current_month_slice_version || null,
    current_month: data.current_month || rawPayload?.current_month || null,
    generated_at: data.generated_at || rawPayload?.generated_at || null,
  });
  return { payload, rawPayload, generatedAt: data.generated_at || null };
}

async function writePortalPayloadDbRow(writeRow, updateExisting) {
  return updateExisting
    ? retryOnStatementTimeout(async () => admin.from('portal_payload').update(writeRow).eq('id', 1))
    : retryOnStatementTimeout(async () => admin.from('portal_payload').upsert(writeRow));
}

export async function writePortalPayload(payload, { generatedAt = new Date().toISOString(), updateExisting = false } = {}) {
  const writeRow = buildPortalPayloadWriteRow(payload, generatedAt);
  let result = await writePortalPayloadDbRow(writeRow, updateExisting);
  if (result?.error && isPortalPayloadSidecarSchemaError(result.error.message)) {
    console.warn('[portalPayload] portal_payload sidecar columns unavailable; retrying legacy payload-only write:', result.error.message);
    result = await writePortalPayloadDbRow({
      id: 1,
      generated_at: generatedAt,
      payload: writeRow.payload,
    }, updateExisting);
  }
...
function historicalMonthsFromPayload(payload, { excludeMonth = null } = {}) {
  if (!payload || typeof payload !== 'object') return [];
  const excluded = isValidMonthKey(excludeMonth) ? excludeMonth : null;
  const months = Array.isArray(payload.months)
    ? payload.months.filter(isValidMonthKey)
    : [];
  const monthlyKeys = payload.monthly && typeof payload.monthly === 'object'
    ? Object.keys(payload.monthly).filter(isValidMonthKey)
    : [];
  return [...new Set([...months, ...monthlyKeys])]
    .filter((month) => month !== excluded)
    .sort();
}

function analyzeStoredHistoryForRead(payload, { excludeMonth = null } = {}) {
  const structuralRepairMonths = summarizeHistoricalMonthlyCoverage(payload, { excludeMonth }).repairMonths;
  if (!payload || typeof payload !== 'object') return structuralRepairMonths;
  if (payload.build_version === PORTAL_PAYLOAD_BUILD_VERSION) return structuralRepairMonths;
  return [...new Set([
    ...historicalMonthsFromPayload(payload, { excludeMonth }),
    ...structuralRepairMonths,
  ])].sort();
}
...
async function repairStoredHistoricalSlicesForRead(payload, { excludeMonth = null } = {}) {
  const repairMonths = analyzeStoredHistoryForRead(payload, { excludeMonth });
...
async function getSharedStoredHistoricalRepair(payload, generatedAt, excludeMonth = null) {
  ...
  const promise = repairStoredHistoricalSlicesForRead(payload, { excludeMonth })
    .then((result) => {
...
export function mergeFreshCurrentMonth(storedPayload, livePayload) {
  ...
  const historicalBuildVersion = Object.prototype.hasOwnProperty.call(storedPayload, 'build_version')
    ? (storedPayload.build_version ?? null)
    : null;
  ...
  return {
    ...
    build_version: historicalBuildVersion,
...
export function payloadFromLiveCurrent(livePayload) {
  ...
  return {
    ...
    current_month: currentMonth,
    prev_month: derivePrevMonthKey(currentMonth),
...
function payloadFromReadableCurrentSlice(payload, generatedAt = null) {
  ...
  return {
    ...
    sites: payload.sites,
    totals: payload.totals,
    monthly: currentMonth ? { [currentMonth]: payload.sites } : {},
    history: currentMonth ? [buildHistoryPoint(currentMonth, payload.sites)] : [],
  };
}

function reusableLastGoodCurrentPayload(currentMonthKey) {
  if (sharedLastGoodCurrentPayload.monthKey !== currentMonthKey || !sharedLastGoodCurrentPayload.result) return null;
  if ((Date.now() - sharedLastGoodCurrentPayload.resultAtMs) > LAST_GOOD_CURRENT_PAYLOAD_REUSE_MS) return null;
  return sharedLastGoodCurrentPayload.result;
}

function rememberLastGoodStoredPayload(result) {
  if (!result?.payload) return result;
  sharedLastGoodStoredPayload = {
    result,
    resultAtMs: Date.now(),
  };
  return result;
...
function reusableLastGoodStoredPayload() {
  if (!sharedLastGoodStoredPayload.result?.payload) return null;
  if ((Date.now() - sharedLastGoodStoredPayload.resultAtMs) > LAST_GOOD_STORED_PAYLOAD_REUSE_MS) return null;
  return sharedLastGoodStoredPayload.result;
}

function rememberLastGoodCurrentPayload(currentMonthKey, result) {
  if (!result?.payload || result.payload.current_month !== currentMonthKey) return result;
  sharedLastGoodCurrentPayload = {
...
function makeCurrentPayloadResult(currentMonthKey, payload, generatedAt, { degraded = false, reason = null } = {}) {
  return rememberLastGoodCurrentPayload(currentMonthKey, {
    payload,
    generatedAt: generatedAt || null,
    freshness_degraded: degraded,
    freshness_reason: degraded ? (reason || 'unknown') : null,
  });
}
...
export async function readPortalPayloadFreshCurrentMonth(now = new Date()) {
  ...
  const cachedCurrentPayload = reusableLastGoodCurrentPayload(currentMonthKey);
  const cachedStoredPayload = reusableLastGoodStoredPayload();
  let liveWarmupTimer = null;
  let scheduledLiveCurrentPromise = null;
  let storedCurrentSliceWarmupTimer = null;
  let scheduledStoredCurrentSlicePromise = null;
  const clearLiveWarmupTimer = () => {
    if (!liveWarmupTimer) return;
    clearTimeout(liveWarmupTimer);
    liveWarmupTimer = null;
  };
  const clearStoredCurrentSliceWarmupTimer = () => {
    if (!storedCurrentSliceWarmupTimer) return;
    clearTimeout(storedCurrentSliceWarmupTimer);
    storedCurrentSliceWarmupTimer = null;
  };
  const ensureLiveCurrentStarted = () => {
    if (scheduledLiveCurrentPromise) return scheduledLiveCurrentPromise;
    clearLiveWarmupTimer();
    scheduledLiveCurrentPromise = readLiveCurrentWithTimeout(currentMonthStart);
    scheduledLiveCurrentPromise.catch(() => {});
    return scheduledLiveCurrentPromise;
  };
  const ensureStoredCurrentSliceStarted = () => {
    if (scheduledStoredCurrentSlicePromise) return scheduledStoredCurrentSlicePromise;
    clearStoredCurrentSliceWarmupTimer();
    scheduledStoredCurrentSlicePromise = withOperationTimeout(
      readPortalPayloadCurrentSlice(),
      STORED_PORTAL_CURRENT_SLICE_READ_TIMEOUT_MS,
      'stored portal_payload current-slice read',
    );
    scheduledStoredCurrentSlicePromise.catch(() => {});
    return scheduledStoredCurrentSlicePromise;
  };
  liveWarmupTimer = setTimeout(() => {
    ensureLiveCurrentStarted();
  }, LIVE_CURRENT_WARMUP_DELAY_MS);
  storedCurrentSliceWarmupTimer = setTimeout(() => {
    ensureStoredCurrentSliceStarted();
  }, STORED_CURRENT_SLICE_WARMUP_DELAY_MS);

  let stored = null;
  let storedCurrentSlice = null;
  let liveBuildFailed = false;
  let storedReadFailed = false;
  let storedReadError = null;
  let storedSource = null;
  try {
    try {
      stored = await withOperationTimeout(readPortalPayload(), STORED_PORTAL_READ_TIMEOUT_MS, 'stored portal_payload read');
      if (stored?.payload) {
        storedSource = 'db';
        stored = rememberLastGoodStoredPayload(stored);
      }
    } catch (error) {
      storedReadFailed = true;
      storedReadError = error;
      console.warn('[portalPayload] stored portal_payload read failed; serving live current-month fallback:', error?.message || error);
      if (cachedStoredPayload?.payload) {
        storedSource = 'cache';
        stored = cachedStoredPayload;
      }
      // If this warm instance already served a verified current-month payload recently, prefer that
      // known-good result over forcing every subsequent request to wait for the same struggling DB
      // path again. Healthy reads still bypass this entirely; it is only a failure-mode fallback.
      if (!stored?.payload && cachedCurrentPayload) {
        return rememberLastGoodCurrentPayload(currentMonthKey, {
          ...cachedCurrentPayload,
          freshness_degraded: true,
          freshness_reason: 'cached-current-fallback',
        });
      }
      if (!stored?.payload) {
        try {
          storedCurrentSlice = await ensureStoredCurrentSliceStarted();
        } catch (currentSliceError) {
          console.warn('[portalPayload] stored current-slice fallback read failed:', currentSliceError?.message || currentSliceError);
        }
      }
    }
    ...
    if (stored?.payload) {
      ...
      if (storedSource === 'db') rememberLastGoodStoredPayload(stored);
    }
    ...
    const storedCurrentSliceTrustedShape = storedPayloadCurrentRich && (storedBuildVersionCurrent || storedCurrentSliceVersionCurrent);
    let sourceRefreshActive = false;
    try {
      const lock = await withOperationTimeout(
        checkPullLock({ activeKinds: ['pull', 'rebuild'] }),
        LIVE_REFRESH_LOCK_PROBE_TIMEOUT_MS,
        'current-month lock probe',
      );
      sourceRefreshActive = !!lock?.locked;
    } catch (error) {
      console.warn('[portalPayload] current-month lock probe failed; serving stored portal payload instead of rebuilding during possible source churn:', error?.message || error);
      sourceRefreshActive = true;
    }
    if (sourceRefreshActive && storedPayloadCurrent && stored?.generatedAt) {
      return makeCurrentPayloadResult(currentMonthKey, stored.payload, stored.generatedAt, {
        degraded: true,
        reason: 'active-refresh-fallback',
      });
    }
    if (storedPayloadCurrent && storedCurrentSliceTrustedShape && stored?.generatedAt) {
      try {
        const latestRawPulledAt = await withOperationTimeout(
          latestCurrentMonthRawPulledAt(currentMonthStart),
          RAW_FRESHNESS_PROBE_TIMEOUT_MS,
          'current-month freshness probe',
        );
        ...
        if (storedCurrentSliceVersionCurrent && storedPayloadCurrentRich) {
          return makeCurrentPayloadResult(currentMonthKey, stored.payload, stored.generatedAt, {
            degraded: true,
            reason: 'freshness-probe-fallback',
          });
        }
      }
    }
    const storedCurrentSlicePayloadCurrent = storedCurrentSlice?.payload?.current_month === currentMonthKey
      && hasReadablePortalCurrentSlice(storedCurrentSlice?.payload);
    const storedCurrentSlicePayloadRich = storedCurrentSlice?.payload?.current_month === currentMonthKey
      && hasRichPortalCurrentSlice(storedCurrentSlice?.payload);
    const storedCurrentSliceBuildVersionCurrent = storedCurrentSlice?.payload?.build_version === PORTAL_PAYLOAD_BUILD_VERSION;
    const storedCurrentSliceMarkerCurrent = storedCurrentSlice?.payload?.current_month_slice_version === CURRENT_MONTH_SLICE_VERSION;
    const storedFallbackCurrentSliceTrustedShape = storedCurrentSlicePayloadRich
      && (storedCurrentSliceBuildVersionCurrent || storedCurrentSliceMarkerCurrent);
    if (sourceRefreshActive && storedFallbackCurrentSliceTrustedShape && storedCurrentSlice?.generatedAt) {
      const currentSlicePayload = payloadFromReadableCurrentSlice(storedCurrentSlice.payload, storedCurrentSlice.generatedAt);
      if (currentSlicePayload) {
        return makeCurrentPayloadResult(currentMonthKey, currentSlicePayload, storedCurrentSlice.generatedAt, {
          degraded: true,
          reason: 'active-refresh-current-slice-fallback',
        });
      }
    }
    if (!stored?.payload && storedFallbackCurrentSliceTrustedShape && storedCurrentSlice?.generatedAt) {
      const currentSlicePayload = payloadFromReadableCurrentSlice(storedCurrentSlice.payload, storedCurrentSlice.generatedAt);
      if (currentSlicePayload) {
        try {
          const latestRawPulledAt = await withOperationTimeout(
          ...
          const storedAtMs = new Date(storedCurrentSlice.generatedAt).getTime();
          const latestRawAtMs = latestRawPulledAt ? new Date(latestRawPulledAt).getTime() : 0;
          const storedCoversLatestRaw = !latestRawPulledAt || (Number.isFinite(storedAtMs) && Number.isFinite(latestRawAtMs) && storedAtMs >= latestRawAtMs);
          if (storedCoversLatestRaw) {
            return makeCurrentPayloadResult(currentMonthKey, currentSlicePayload, storedCurrentSlice.generatedAt, {
              degraded: true,
              reason: 'stored-current-slice-fallback',
            });
          }
        } catch (error) {
          console.warn('[portalPayload] current-slice freshness probe failed; serving stored current-slice fallback:', error?.message || error);
          if (storedCurrentSliceMarkerCurrent && storedCurrentSlicePayloadCurrent) {
            return makeCurrentPayloadResult(currentMonthKey, currentSlicePayload, storedCurrentSlice.generatedAt, {
              degraded: true,
              reason: 'stored-current-slice-probe-fallback',
            });
          }
        }
      }
    ...
    try {
      ...
      liveCurrent = await ensureLiveCurrentStarted();
    } catch (error) {
      ...
      if (!stored?.payload) {
        try {
          const recoveredStored = await withOperationTimeout(
            readPortalPayload(),
            STORED_PORTAL_READ_TIMEOUT_MS,
            'stored portal_payload retry',
          );
          if (recoveredStored?.payload) {
            storedSource = 'db';
            stored = rememberLastGoodStoredPayload(recoveredStored);
          }
        } catch (retryError) {
      ...
      if (!stored?.payload && !storedCurrentSlice?.payload) {
        try {
          storedCurrentSlice = await ensureStoredCurrentSliceStarted();
        } catch (currentSliceRetryError) {
          console.warn('[portalPayload] stored current-slice retry after live current build failure also failed:', currentSliceRetryError?.message || currentSliceRetryError);
        }
      }
      if (!stored?.payload && cachedCurrentPayload) {
        return rememberLastGoodCurrentPayload(currentMonthKey, {
      ...
      if (!stored?.payload && !storedCurrentSlice?.payload) throw error;
    }

    if (liveCurrent && storedReadFailed && !stored?.payload) {
      const initialStoredReadTimedOut = String(storedReadError?.message || '').includes('stored portal_payload read exceeded');
      if (initialStoredReadTimedOut) {
        console.warn('[portalPayload] skipping stored portal_payload retry after initial timeout; returning current-month-only fallback until stored metadata recovers.');
      } else {
        try {
          const recoveredStored = await withOperationTimeout(
            readPortalPayload(),
    ...
    const payload = mergeBasePayload
      ...
      : (liveBuildFailed
          ? payloadFromReadableCurrentSlice(stored?.payload || storedCurrentSlice?.payload, stored?.generatedAt || storedCurrentSlice?.generatedAt)
          : payloadFromLiveCurrent(liveCurrent));
    if (!payload) return null;
    return makeCurrentPayloadResult(currentMonthKey, payload, payload?.generated_at || stored?.generatedAt || null, {
      degraded: liveBuildFailed || !mergeBasePayload || storedSource === 'cache',
      reason: liveBuildFailed
        ? 'live-build-fallback'
        : (!mergeBasePayload ? 'live-current-only' : (storedSource === 'cache' ? 'cached-stored-base' : null)),
    });
  } finally {
    clearLiveWarmupTimer();
    clearStoredCurrentSliceWarmupTimer();
...
export async function readPortalPayloadFreshCurrentMonthStable(now = new Date(), opts = {}) {
  ...
  for (let attempt = 0; attempt < attempts; attempt++) {
    try {
      const result = await readPortalPayloadFreshCurrentMonth(now);
      if (result?.payload && result.freshness_degraded !== true) return result;
      lastResult = result;
    } catch (error) {
      lastError = error;
    }
  ...
  if (lastResult?.payload) return lastResult;
