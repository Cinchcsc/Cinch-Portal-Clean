...
async function pullPeriodForSite(loc, { start, end }) {
  ...
  // in one extra day beyond `end` (to satisfy SiteLink), so cut mio.rows back to the true [start, end]
  // window by MoveDate before handing off to the unmodified shared parser.
  const startDay = dayOnly(start), endDay = dayOnly(end);
  const trimmedMioRows = mio.rows.filter((r) => {
    if (!r.MoveDate) return false;
    const d = new Date(r.MoveDate);
