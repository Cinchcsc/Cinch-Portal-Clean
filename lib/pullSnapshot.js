...
// (see probe-daily-granularity.js) if a reliable per-row date field turns up for client-side slicing.
//
...
// pick) is NOT yet included — pending probe-daily-granularity.js confirming whether InquiryTracking
// carries a usable target-move-in-date field. Add once confirmed; every other metric here doesn't
// need it.
//
...
async function pullPeriodForSite(loc, { start, end }) {
  ...
  return {
    ...
    reservationBacklog: null,   // TODO: pending probe-daily-granularity.js — see file header
    moveIns: mo.move_ins,
