...
import { reportingCurrentMonthStart } from './reportingPeriod.js';
...
// EXPORTED 28 Jul 2026: lib/rebuildPayload.js's production-hardening pass needs a stable list of
// every known site code (to detect historical months missing a site) and a build-version tag (to
// detect payloads written by an older code shape that need a full rebuild rather than a cheap
// current-month merge) — both derived from data that already lived in this file.
export const PORTAL_PAYLOAD_BUILD_VERSION = '2026-07-27-history-aware-v1';
...
function recordFor(code, name, c, full, useBillingAdjustedRate = false, currentMonthData = null) {
  ...
  const rec = {
    ...
    debtors: {
      ...
      // "allOverdue" is the broad/raw bucket kept for ad hoc analysis only, distinct from the
      // 30+ day KPI fields above. Prefer PastDueBalances' true any-age total when present; falling
      // back to the 30+ ManagementSummary figure first made this field silently narrower than its
      // own label on any month where both sources existed.
      allOverdue: pd.total_overdue ?? (mg.delinquent_30plus_total ?? (pd.total_overdue_30plus ?? 0)),
...
export async function buildPayload(currentMonth, prevMonth) {
  ...
  const generatedAt = [cur, prev]
    ...
    .sort()
    .slice(-1)[0] || null;
  return { generated_at: generatedAt, current_month: cur, prev_month: prev, months: visibleMonths, sites, totals, history: buildHistory(visibleMonths, monthly), monthly };
}
...
function mergeSiteAcrossRange(recs) {
  ...
  rec.debtors.ageing = (() => {
    const keys = ['31-60', '61-90', '91-120', '121-180', '181-360', '361+'];
    const out = {};
...
export async function buildPayloadRange(fromMonth, toMonth) {
  ...
  if (!rangeMonths.length) {
    return { generated_at: latestPulledAt, current_month: requestedTo, prev_month: null, months: visibleMonths, sites: [], totals: null, history: [], monthly: {}, range: { from, to: requestedTo, months: rangeMonths } };
  }
  ...
  const totals = aggregateTotals(sites);
  return {
    generated_at: latestPulledAt, current_month: rangeMonths[rangeMonths.length - 1], prev_month: rangeMonths.length >= 2 ? rangeMonths[rangeMonths.length - 2] : null,
    months: visibleMonths, sites, totals, history: [], monthly: {}, range: { from, to: requestedTo, months: rangeMonths },
  };
}
