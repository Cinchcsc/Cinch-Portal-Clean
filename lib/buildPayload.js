...
function recordFor(code, name, c, full, useBillingAdjustedRate = false) {
  ...
  const rec = {
    ...
    enquiries: {
      ...
      // reservationConversions (Enquiry -> Reservation) — FIXED AGAIN 23 Jul 2026
      // (production-readiness audit): the legacy portal's conversion widget lines up far more closely
      // with InquiryTracking's OWN aggregate InquirySource table (`iConverted` / `iTotal`) than with
      // the per-row Activity-table reservation-stage count this project had been using. Keep the
      // separate Enquiries by Channel widgets on the Activity-table counts above (better channel
      // detail), but source the conversion numerator+base from InquirySource so the conversion widget,
      // by-store conversion %, and YoY conversion trend all follow the same aggregate legacy-style
      // definition.
      // Replaces a whole family of per-lead cohort-matching attempts (6-17 Jul 2026: email hash, then
      // phone added task #301, then a same/previous-month lookback window added task #303 — see git
      // history for the full run of iterations) that were all trying to trace an individual lead from
      // their enquiry row to a later reservation row. Michael pinned down legacy's real June 2026
      // portfolio rate at 19.8% (confirmed live) — none of the cohort-matched versions got anywhere
      // close (low single digits up to ~10%). Frozen-raw-response rechecks on July 23, 2026
      // (probe-reservation-reparse-check.js) later confirmed our OWN stored June ratio on this plain
      // count-ratio definition is ~14.4-14.5%, so an older 23.9% note in project history was stale.
      // The durable conclusion is still the same: legacy isn't tracking individual leads here at all —
      // just dividing two independent aggregates. Also fixes the live/in-progress month always looking artificially low:
      // a period-ratio needs no cross-month data at all, so there's no "next month doesn't exist yet"
      // gap to wait out.
      reservationConversions: lf.inquiry_source_converted ?? lf.reservation_stage_count ?? 0,
      reservationConversionBase: lf.inquiry_source_total ?? lf.total_enquiries ?? 0,
  ...
  rec.moveInVarianceCount = mio.move_ins || 0;
  ...
  rec.moveInVarStdRateActualPct = +(rec.moveInVarStdRatePct - 8.33).toFixed(2);
...
export async function buildPayload(currentMonth, prevMonth) {
  ...
  for (let mi = 0; mi < months.length; mi++) {
    ...
    monthly[mk] = Object.keys(idx).filter(code => idx[code][mk] && idx[code][mk].occupancy && idx[code][mk].occupancy.total_units > 0)
      .map(code => {
        const rec = recordFor(code, nameOf[code] || code, idx[code][mk], false, mk === cur);
  ...
  const generatedAt = [cur, prev]
    ...
    .sort()
    .slice(-1)[0] || null;
  return { generated_at: generatedAt, current_month: cur, prev_month: prev, months, sites, totals, history: buildHistory(months, monthly), monthly };
}
...
function aggregateTotals(sites) {
  ...
  totals.moveInVarStdRateActualPct = +(totals.moveInVarStdRatePct - 8.33).toFixed(2);
...
function buildHistory(months, monthly) {
  return months.map((mk) => {
    ...
    // reservationConversions/reservationConversionBase for THIS month is already correct with no
    ...
    return {
      ...
      enqTotal, enqReservationConversions, enqReservationConversionBase,   // Marketing YoY trend (task #136) — channel-count enquiries + InquirySource conversion aggregate, portfolio-wide
      ...
      enqConvPct: enqReservationConversionBase ? +(enqReservationConversions / enqReservationConversionBase * 100).toFixed(1) : 0,
    };
  });
}
...
export async function buildPayloadRange(fromMonth, toMonth) {
  ...
  if (!rangeMonths.length) {
    return { generated_at: latestPulledAt, current_month: to, prev_month: null, months, sites: [], totals: null, history: [], monthly: {}, range: { from, to, months: rangeMonths } };
  }
  ...
  const sites = codes.map((code) => {
    const recs = rangeMonths.map((mk) => {
        const rec = recordFor(code, nameOf[code] || NAMES[code] || code, (idx[code] && idx[code][mk]) || {}, true, mk === realCurrentMonth);
        applyAutobillDailyAverage(rec, mk, autobillDailyMap);
        return rec;
      });
    return mergeSiteAcrossRange(recs);
  });
  ...
  const totals = aggregateTotals(sites);
  return {
    generated_at: latestPulledAt, current_month: to, prev_month: rangeMonths.length >= 2 ? rangeMonths[rangeMonths.length - 2] : null,
