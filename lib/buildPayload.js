...
// `prevC` (optional) is the PRECEDING month's raw bundle — only used for the reservationConversions
// previous-month lookback match below; undefined is fine everywhere else (same-month-only matching).
// CHANGED 17 Jul 2026 (task #303, Michael: "if they enquired last month, but moved in this month, it
// should carry over to this month") — this used to be `nextC`, the FOLLOWING month, so a conversion
// was attributed to the ENQUIRY's month by looking forward for a later reservation. That can never
// see the live in-progress month's own eventual conversions (next month doesn't exist yet), which is
// why the current month's rate always looked artificially low until the following month closed.
// Flipped to attribute a conversion to the RESERVATION's month instead, looking back at enquiries from
// that month or the one before — both of which already exist today, even for the live current month.
// This doesn't double-count: a lead who enquires in month M and reserves in M+1 used to be counted
// once, in M (via the old forward lag-match); now it's counted once, in M+1 (via the new lookback) —
// same single event, just attributed to the month it actually closed in rather than the month it
// started in, which also happens to be the only version that's fully knowable in real time.
function recordFor(code, name, c, full, prevC) {
  ...
  const rec = {
    ...
    enquiries: {
      ...
      //   4. Email-hash cross-reference (lead_funnel's inquiry_contacts vs move_ins_outs'
      ...
      // the conversion rate as "still looks low"): also counted NEXT-MONTH lag matches — an enquiry
      ...
      // next-month lag brought it to 4.0% (162/4004) — real additional signal, not double-counting
      // (a lead only ever has one Reservation-stage row, so it can only match one of the two sets).
      // That version fell back to same-month-only for the live/in-progress current month, since next
      // month's data doesn't exist yet — meaning the current month's own rate always looked
      // artificially low until the following month closed and could look back at it.
      // EXTENDED 17 Jul 2026 (task #301, Michael: "add phone and walk ins both as matching keys"):
      // added phone as a second identifier alongside email (see reportMap.js's phoneHash comment for
      // why — live data showed the "phone/walk-in leads have no email" assumption behind the original
      // fix wasn't actually true for this account, so this is really about catching typo'd/changed
      // contact details between an enquiry and its later reservation, not filling an email gap).
      // FLIPPED 17 Jul 2026 (task #303, Michael: "if they enquired last month, but moved in this
      // month, it should carry over to this month") — replaced the next-month lag-match above with a
      // PREVIOUS-month lookback: a conversion is now attributed to the month the RESERVATION happened
      // in, matched against enquiries from that same month OR the one before, instead of being
      // attributed to the month the ENQUIRY happened in and looking forward for a later reservation.
      // Walked from the reservation side now (one {emailHash,phoneHash} object per reservation-stage
      // row, own PII-safety rule as inquiry_contacts) so a reservation matching on both email and
      // phone — or matching an enquiry from both this month and last month — still only counts once.
      // Doesn't double-count vs. the old version: a lead who enquires in month M and reserves in M+1
      // used to be counted once in M (old forward lag-match); now it's counted once in M+1 (new
      // lookback) — same single event, reattributed to the month it actually closed in. The practical
      // win is that this is the only version fully knowable for the LIVE month: `prevC` (last month)
      // already exists today, unlike a "next month" for the month that's still in progress.
      reservationConversions: (() => {
        const resContacts = lf.reservation_contacts;
        if (!Array.isArray(resContacts) || !resContacts.length) return lf.reservations || 0;
        const enqHashes = new Set();
        for (const c of (lf.inquiry_contacts || [])) { if (c.emailHash) enqHashes.add(c.emailHash); if (c.phoneHash) enqHashes.add(c.phoneHash); }
        for (const c of (((prevC && prevC.lead_funnel) || {}).inquiry_contacts || [])) { if (c.emailHash) enqHashes.add(c.emailHash); if (c.phoneHash) enqHashes.add(c.phoneHash); }
        return resContacts.filter((c) => (c.emailHash && enqHashes.has(c.emailHash)) || (c.phoneHash && enqHashes.has(c.phoneHash))).length;
      })(),
...
export async function buildPayload(currentMonth, prevMonth) {
  ...
  for (let mi = 0; mi < months.length; mi++) {
    const mk = months[mi], prevMk = months[mi - 1];
    monthly[mk] = Object.keys(idx).filter(code => idx[code][mk] && idx[code][mk].occupancy && idx[code][mk].occupancy.total_units > 0)
      .map(code => {
        const rec = recordFor(code, nameOf[code] || code, idx[code][mk], false, prevMk ? idx[code][prevMk] : undefined);
  ...
  // FULL detail for the current month. FIXED 17 Jul 2026 (task #303): this used to pass no prevC at
  // all (reservationConversions was still same-month-only lag-matching from the ENQUIRY side back
  // then, so a "previous month" argument wouldn't have helped). Now that recordFor() attributes a
  // conversion to the RESERVATION's month and looks back one month, the live/in-progress current
  // month needs its prevC too — `prev` (this function's own second argument) is already exactly
  // last month, so this is the one month whose lookback data is guaranteed to already exist.
  ...
  const sites = Object.keys(idx).filter(code => idx[code][cur] && idx[code][cur].occupancy && idx[code][cur].occupancy.total_units > 0)
    .map(code => {
      const rec = recordFor(code, nameOf[code] || code, idx[code][cur], true, idx[code][prev]);
...
export async function buildPayloadRange(fromMonth, toMonth) {
  ...
  // Only ask the DB for months this call can possibly use: one extra calendar month BEFORE `from` so
  // the reservationConversions previous-month lookback match (below) still has its data for the FIRST
  // month in range, through `to` itself. FLIPPED 17 Jul 2026 (task #303) from fetching one extra month
  // PAST `to` — that supported the old next-month lag-match (attributed a conversion to the enquiry's
  // month, looking forward); recordFor() now attributes to the reservation's month and looks backward,
  // so the extra month needed is before the range, not after it. See fetchAllRaw()'s comment for why
  // fetching outside the exact requested range matters here.
  const beforeFrom = new Date(fromMonth.getFullYear(), fromMonth.getMonth() - 1, 1);
  const afterTo = new Date(toMonth.getFullYear(), toMonth.getMonth() + 1, 1);
  const monthRange = {
  ...
  const sites = codes.map((code) => {
    // prevC for the lookback match uses the GLOBAL month series (not just rangeMonths) so a range's
    // FIRST month still gets a correct lookback match against the month right before it, even if that
    // prior month falls outside the selected range. FLIPPED 17 Jul 2026 (task #303) from a nextMk/
    // "lag-match" lookup — see recordFor()'s own comment for the full reasoning.
    const recs = rangeMonths.filter((mk) => idx[code][mk] && idx[code][mk].occupancy && idx[code][mk].occupancy.total_units > 0)
      .map((mk) => {
        const prevMk = months[months.indexOf(mk) - 1];
        const rec = recordFor(code, nameOf[code] || code, idx[code][mk], true, prevMk ? idx[code][prevMk] : undefined);
        applyAutobillDailyAverage(rec, mk, autobillDailyMap);
        return rec;
      });
  ...
  // selected month that is ALSO the current in-progress month (months[months.length-1] is always
  // that month, since pull.js only ever re-pulls the current month) — an explicitly-selected PAST
  // month already has its own correct, complete data and needs no substitution, and a genuine
  // multi-month range's partial final month is a different, accepted characteristic of ranges.
  const isSingleCurrentMonth = rangeMonths.length === 1 && rangeMonths[0] === months[months.length - 1];
