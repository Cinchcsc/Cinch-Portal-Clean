...
function recordFor(code, name, c, full, nextC) {
  ...
  const rec = {
    ...
    enquiries: {
      ...
      // lf.inquiry_contacts is now one {emailHash,phoneHash} object per enquiry row (was a flat email-
      // hash array) — a row counts as converted if EITHER of its two hashes shows up in the combined
      // reservation-contact pool, checked once per row so matching on both doesn't double-count.
      reservationConversions: (() => {
        const contacts = lf.inquiry_contacts;
        if (!Array.isArray(contacts) || !contacts.length) return lf.reservations || 0;
        const resHashes = new Set(lf.reservation_contact_hashes || []);
        const nextResHashes = new Set(((nextC && nextC.lead_funnel) || {}).reservation_contact_hashes || []);
        const matched = (h) => h && (resHashes.has(h) || nextResHashes.has(h));
        return contacts.filter((c) => matched(c.emailHash) || matched(c.phoneHash)).length;
      })(),
...
export async function buildPayload(currentMonth, prevMonth) {
  ...
  // data yet, so its own reservationConversions falls back to same-month-only.
