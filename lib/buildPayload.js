...
function recordFor(code, name, c, full, nextC) {
  ...
  // Real Rate — REPLACED 8 Jul 2026, matching the EXACT formula validated earlier today across
  // multiple sites (scripts/probe-rate-both-formulas.js, per legacy's own tooltip text captured in
  // probe-truerate-tooltip.js): Σ(TruePeriod − ThisPeriodAdjustments) ÷ Σ TOTAL AREA × 12 — using the
  // True Revenue custom report (#781861)'s by_type rows for the numerator, and CRITICALLY the RentRoll
  // TOTAL area INCLUDING VACANT units (rr.total_area_all_units) for the denominator, NOT occupied
  // area like every other rate calc in this file (this was the mistake in the first pass at this fix —
  // occupied-area landed ~3-4x too high, matching exactly what was reported). Total variant sums every
  // by_type row; Self Storage variant (below) sums only "Self Storage" row(s) (substring match, same
  // as reportMap.js's own isSS(), in case True Revenue ever labels a row "Indoor Self Storage" too).
  // Falls back to the old dcRent-based figure (reverse-derived into an equivalent numerator) when
  // true_revenue wasn't pulled for this site/month, so portfolio sum-then-divide and Month-on-Month
  // trend lines keep working across mixed-availability months.
  // CAVEAT: True Revenue's own truePeriod figures are under a separate, still-open investigation
  // (task #77, suspected ~2.14x inflation) — if that's confirmed real, this Real Rate inherits it.
  ...
  const trueRevenueNumerator = hasTrueRevenue
    ? byType.reduce((a, r) => a + (r.truePeriod || 0) - (r.adj || 0), 0)
    : realRateFallback * totalArea / 12;
  const realRate = totalArea ? R2(trueRevenueNumerator / totalArea * 12) : realRateFallback;
  ...
  // Self Storage Real Rate — same True Revenue-based replacement as Total Real Rate above, scoped to
  // just the "Self Storage" by_type row(s), divided by Self Storage's TOTAL area (incl. vacant).
  ...
  const ssTrueRevenueNumerator = hasTrueRevenue
    ? byType.filter((r) => String(r.desc || '').toLowerCase().includes('self storage')).reduce((a, r) => a + (r.truePeriod || 0) - (r.adj || 0), 0)
    : ssRealFallback * ssArea / 12;
  const ssReal = ssArea ? R2(ssTrueRevenueNumerator / ssArea * 12) : ssRealFallback;
