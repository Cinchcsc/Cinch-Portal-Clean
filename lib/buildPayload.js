...
function recordFor(code, name, c, full, nextC) {
  ...
  // elapsed days, not bucketed by month). The math itself checks out (verified: every site's realRate
  // moved by a uniform ~3.04x, exactly 365/10 / 12 — no bug), but it made the gap vs Michael's legacy
  // targets MUCH worse (avg error 26% -> 207%) instead of better, uniformly in the same direction
  // across all 25 sites. That means legacy's "Jul 2026" Real Rate figure is NOT a properly-annualized
  // July-month-to-date number the way this assumed — it's most likely still showing June's (the last
  // COMPLETE month's) figure under the current period, or sourced differently altogether. REVERTED to
  // the plain x12 here (empirically closer to legacy despite being "wrong" for a different reason)
  // until that's confirmed with Michael. period_days is still computed/carried (see
  // trueRevenuePeriodDays below) so it's ready to use the moment we know what legacy actually shows —
  // just not driving the displayed number yet.
