...
//
// 11 Aug 2026 (historical integrity audit): recent closed-month `payload.monthly` slices were proven
// stale across many stores even though the row still looked structurally complete. The daily fast
// path had no semantic marker strong enough to force those older months back through raw_report after
// later formula/field additions, so the default unscoped portal metadata could keep serving zeros/old
// values for move-ins, reservations, enquiries, and other historical fields while explicit ranged
// reads were correct. This version bump intentionally forces a one-time historical slice repair.
export const PORTAL_PAYLOAD_BUILD_VERSION = '2026-08-11-historical-slice-repair-v1';

const monthParts = (d) => ({ year: d.getFullYear(), month: d.getMonth() + 1 });
const ym = (d) => {
...
async function fetchAutobillDailyMap(monthRange) {
  try {
    ...
    let lastId = 0;
    for (;;) {
      let q = admin.from('autobill_daily').select('id,site_code,month,sample_date,pct').gt('id', lastId).order('id').limit(PAGE);
    ...
    for (const r of out) {
      ...
      const sampleDate = String(r.sample_date || '');
      const startDay = bounds ? formatLocalYmd(bounds.start) : null;
      const endDay = bounds ? formatLocalYmd(bounds.end) : null;
      // Ignore drifted samples that claim to belong to month M but were stored against a business
      // date outside month M's real visible window. This protects Autobill Conversion from stale
      // rows written before the represented-day cap was fully stabilized.
      if (!bounds || !/^\d{4}-\d{2}-\d{2}$/.test(sampleDate) || sampleDate < startDay || sampleDate > endDay) continue;
...
async function buildIndex(monthRange, opts = {}) {
  const liveCurrentFast = !!opts.liveCurrentFast;
  // Current-month cold-start hardening (10 Aug 2026): the sites reference lookup and the main
  // raw_report scan are independent. Running them serially adds avoidable wall-clock time to the
  // exact read path that is already flirting with the 90s live-current timeout during Supabase
  // turbulence, so start both immediately and await them together.
  const sitesRefPromise = retryOnStatementTimeout(async () => admin.from('sites').select('code,name'));
  const rowsPromise = fetchAllRaw(monthRange, { liveCurrentFast });
  const [{ data: sitesRef, error: sitesErr }, rows] = await Promise.all([sitesRefPromise, rowsPromise]);
  if (sitesErr) {
  ...
  // Current-month reservation backlog must be reparsed even on the liveCurrentFast path. The fast
  // current-month reader feeds both the default portal response and rebuildPayload()'s cheap
  // merge-current-month path; skipping ReservationList raw here leaves activeReservations and the
  // reservedSqftEstimate backlog snapshot stuck on older parsed JSON even after the rest of the
  // current slice has been refreshed from raw_report.
  const needReservationsRaw = rows.some((r) => r.report === 'reservations' && String(r.month).slice(0, 7) === realCurrentMonth);
  ...
  const rawReportMapsPromise = rawReports.length
    ? fetchMultiReportRawMaps(rawReports, rawFetchMonthRange, liveCurrentFast ? 200 : 50)
    : Promise.resolve({});
  const needCurrentMonthMoveRaw = rows.some((r) => r.report === 'move_ins_outs' && String(r.month).slice(0, 7) === realCurrentMonth);
  // The move_ins_outs raw map is a separate narrow query that feeds only current-month visible flow
  // trimming. Load it alongside the other raw_response maps instead of after them to avoid stacking
  // two independent raw-report waits on top of each other.
  const currentMonthMoveRawMapPromise = needCurrentMonthMoveRaw
    ? fetchMoveInsOutsRawMap({ start: `${realCurrentMonth}-01`, endExclusive: `${nextMonthKey(realCurrentMonth)}-01` })
    : Promise.resolve(null);
  const [rawReportMaps, currentMonthMoveRawMap] = await Promise.all([rawReportMapsPromise, currentMonthMoveRawMapPromise]);
