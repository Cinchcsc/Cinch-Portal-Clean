...
// NOTE (2 Jul 2026, Michael): the new portal intentionally tracks all 27 sites, including the 2 the
// legacy portal doesn't yet — those stores should stay LIVE and included in every widget, not
// excluded. The Offices/Self-Storage/overall-occupancy gap vs. legacy targets is therefore an
// expected, permanent difference in scope (27 vs 25 stores), not something to patch around here.
...
function recordFor(code, name, c, full, nextC) {
  ...
  // Rate / Real Rate (portfolio-wide, all unit types) still come from RentRoll only — the locked
  // spec (Michael 1 Jul 2026: Σ dcRent ÷ Σ area × 12 for Rate, Σ dcStandardRate ÷ Σ area × 12 for
  // Real Rate) is unchanged for these.
  const rent = rr.monthly_rent || 0;
  const rate = rr.rate_per_sqft_ann || 0;
  const realRate = rr.real_rate_per_sqft_ann || 0;
  // Self Storage Rate / Self Storage Real Rate — RentRoll only (Michael, 7 Jul 2026: revert the
  // OccupancyStatistics fallback added 7 Jul 2026; keep RentRoll as the sole source everywhere,
  // including Enfield, even where it disagrees with OccupancyStatistics).
  const ssRate = (rr.self_storage && rr.self_storage.rate_per_sqft_ann) || 0;
  const ssReal = (rr.self_storage && rr.self_storage.real_rate_per_sqft_ann) || 0;
  ...
  const rec = {
    ...
    debtors: {
      // "Delinquent" = balance over 30 days late (R6's own rule) — CONSISTENTLY applied now to
      // every debtor figure below (confirmed bug, 2 Jul 2026: tenantPct/rentRollPct previously used
      // the RAW totals which included the 1-30-day "just invoiced, still in grace period" bucket,
      // inflating both percentages by a large, inconsistent margin vs. `total` below).
      total: pd.total_overdue_30plus ?? (pd.ageing ? Math.round(['31-60', '61-90', '91-120', '121-180', '181-360', '361+'].reduce((a, k) => a + (pd.ageing[k] || 0), 0)) : (pd.total_overdue || 0)),
      accounts: pd.accounts_overdue_30plus ?? pd.accounts_overdue ?? 0,
      allOverdue: pd.total_overdue_30plus ?? pd.total_overdue ?? 0,
      // Debtor Levels widget (legacy portal tooltip, confirmed 2 Jul 2026):
      //   Tenant %   = PastDueBalances Delinquency Units  / Occupancy Statistics Occupied Units
      //   Rent Roll% = PastDueBalances Delinquency Total  / Occupancy Statistics Actual Occupied Unit Rates
      // (the tooltip labels the source "ManagementSummary", but the actual fields live in
      // PastDueBalances/OccupancyStatistics in this pipeline — same data, different report name.)
      tenantPct: occ ? +((pd.accounts_overdue_30plus ?? pd.accounts_overdue ?? 0) / occ * 100).toFixed(1) : 0,
      rentRollPct: occActualRent ? +((pd.total_overdue_30plus ?? pd.total_overdue ?? 0) / occActualRent * 100).toFixed(1) : 0,
    },
    occActualRent,
    ...
    // portal's Marketing page and finding ours at 1,885 vs legacy's 860 for the same month): TOTAL is
    // now phone_leads + walkin_leads + web_leads (ManagementSummary), i.e. literally the same three
    // numbers as the channel tiles beside it, summed — exactly how the legacy portal computes its own
    // "Total Enquiries" tile (confirmed: legacy's Marketing page Total is just its 3 channel tiles
    // added together, nothing more). This REVERTS the "TOTAL stays sourced from InquiryTracking"
    // decision locked 1 Jul 2026 — that source (lead_funnel's total_enquiries) is more complete (it
    // captures leads legacy's own Staff Performance page attributes to an unassigned/automated "CSIL"
    // bucket that legacy's Marketing Total tile never counts), but per explicit instruction the two
    // portals should show directly comparable numbers even at the cost of losing that extra signal.
    // `webOnly`/`email`/`channels` (InquiryTracking-based, used for finer per-channel drill-downs
    // elsewhere, e.g. the Leads by Store table) are unchanged — only this `total` field moved sources.
    enquiries: {
      total: (mg.phone_leads || 0) + (mg.walkin_leads || 0) + (mg.web_leads || 0),
      ...
      phone: mg.phone_leads || 0, walkin: mg.walkin_leads || 0, web: mg.web_leads || 0,
...
// de-dupe/pagination-safety logic instead of a second hand-copied implementation.
...
export async function buildPayload(currentMonth, prevMonth) {
  ...
  for (let mi = 0; mi < months.length; mi++) {
    ...
    monthly[mk] = Object.keys(idx).filter(code => idx[code][mk] && idx[code][mk].occupancy && idx[code][mk].occupancy.total_units > 0)
      .map(code => recordFor(code, nameOf[code] || code, idx[code][mk], false, nextMk ? idx[code][nextMk] : undefined));
  }
  ...
  const sites = Object.keys(idx).filter(code => idx[code][cur] && idx[code][cur].occupancy && idx[code][cur].occupancy.total_units > 0)
    .map(code => recordFor(code, nameOf[code] || code, idx[code][cur], true));
  ...
  sites.sort((a, b) => b.occPC - a.occPC);
...
function aggregateTotals(sites) {
  ...
  // `reservations`/`scheduled_outs` comments and probe:scheduled-outs-historical) — kept here for
  // back-compat/other call sites but no longer used for the "Reservations vs Move-outs" KPI widget.
  ...
  // "Reservations vs Move-outs" widget rebuild (Michael, 6 Jul 2026): both sides are now real
  // historical flow metrics for the given month/range — reservationsMade from lead_funnel
  // (InquiryTracking, confirmed genuinely date-scoped via probe:lead-funnel-reservations) and
  // moveOuts from ManagementSummary (already reliable, powers the working Move-ins & Move-outs
  // widget). Works for every stored month, not just the live one.
...
function buildHistory(months, monthly) {
  return months.map((mk) => {
    ...
    return {
      ...
      rate: s('areaSum') ? R2(s('rentSum') / s('areaSum') * 12) : 0,
      ssRate: s('ssAreaSum') ? R2(s('ssRentSum') / s('ssAreaSum') * 12) : 0,
...
// of 3 monthly totals.
const RANGE_SUM_FIELDS = ['moveIns', 'moveOuts', 'netArea', 'moveOutsYear', 'moveInAreaSum', 'moveInRateSum', 'autobillNewCount', 'autobillNewTotal', 'reservations', 'reservationsMade'];
...
function mergeRowsAcrossMonths(recs, field, keyOf, numericKeys) {
  ...
  for (const rec of recs) for (const row of (rec[field] || [])) {
    const k = keyOf(row);
    const o = (g[k] ??= { ...row });
    if (o !== row) for (const nk of numericKeys) o[nk] = R2((o[nk] || 0) + (row[nk] || 0));
  }
...
function mergeSiteAcrossRange(recs) {
  ...
  rec.customerType = {
    business: { units: bizUnits, pct: 0, rate: bizArea ? R2(bizRent / bizArea * 12) : 0 },
    residential: { units: resUnits, pct: 0, rate: resArea ? R2(resRent / resArea * 12) : 0 },
  };
  rec.enquiries = {
  ...
  rec.rate = rec.areaSum ? R2(rec.rentSum / rec.areaSum * 12) : 0;
  rec.realRate = rec.areaSum ? R2(rec.stdRentSum / rec.areaSum * 12) : 0;
  rec.ssRate = rec.ssAreaSum ? R2(rec.ssRentSum / rec.ssAreaSum * 12) : 0;
  rec.ssReal = rec.ssAreaSum ? R2(rec.ssStdRentSum / rec.ssAreaSum * 12) : 0;
  ...
  const sites = codes.map((code) => {
    ...
    const recs = rangeMonths.filter((mk) => idx[code][mk] && idx[code][mk].occupancy && idx[code][mk].occupancy.total_units > 0)
      .map((mk) => {
        ...
        return recordFor(code, nameOf[code] || code, idx[code][mk], true, nextMk ? idx[code][nextMk] : undefined);
      });
