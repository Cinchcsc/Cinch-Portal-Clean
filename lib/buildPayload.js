...
async function buildIndex(monthRange) {
  const { data: sitesRef } = await admin.from('sites').select('code,name,manager');
  ...
  // managerOf (ADDED 14 Jul 2026, task #174/#207, Facility Groups grouped by manager/team per
  // Michael's decision): null for any site not yet assigned via `npm run set:site-manager` — the
  // frontend buckets those into an "Unassigned" group rather than dropping them.
  const managerOf = Object.fromEntries((sitesRef || []).map(s => [s.code, s.manager || null]));
  ...
  return { idx, nameOf, managerOf, months };
}
...
export async function buildPayload(currentMonth, prevMonth) {
  ...
  const { idx, nameOf, managerOf, months } = await buildIndex();
  ...
  for (let mi = 0; mi < months.length; mi++) {
    ...
    monthly[mk] = Object.keys(idx).filter(code => idx[code][mk] && idx[code][mk].occupancy && idx[code][mk].occupancy.total_units > 0)
      .map(code => {
        ...
        rec.manager = managerOf[code] || null;
...
export async function buildPayloadRange(fromMonth, toMonth) {
  ...
  const { idx, nameOf, managerOf, months } = await buildIndex(monthRange);
