...
function recordFor(code, name, c, full, nextC) {
  ...
  // ANNUALIZE FACTOR — FIXED 10 Jul 2026 (Michael, "keep it live, fix the math"): was always x12,
  // silently assuming tr always covers exactly one complete month. Confirmed via
  // scripts/probe-truerevenue-period-granularity.js that True Revenue's TruePeriod scales linearly
  // with the days actually queried — for the in-progress current month (e.g. July 1-10, 10 of 31
  // days) that's a ~3x understatement once x12'd as if complete. Use the real period length
  // (reportMap.js's true_revenue parser now returns period_days) so a partial month annualizes
  // correctly instead of looking definitively wrong. Falls back to 12 when period_days is unavailable
  // (no true_revenue data at all, or an old cached raw_report row pulled before this field existed).
  const trueRevenuePeriodDays = (hasTrueRevenue && tr.period_days) ? tr.period_days : null;
  const annualizeFactor = trueRevenuePeriodDays ? (365 / trueRevenuePeriodDays) : 12;
...
async function fetchAutobillDailyMap(monthRange) {
  try {
    let q = admin.from('autobill_daily').select('site_code,month,pct');
...
function aggregateTotals(sites) {
  ...
  const occA = sum('occA');
  const claA = sum('claA');
  // Portfolio Real Rate annualize factor — FIXED 10 Jul 2026, same reasoning as recordFor()'s
  // annualizeFactor. Every site in one buildPayload() call is pulled with the same current-month
  // window, so any site's trueRevenuePeriodDays is representative of the whole portfolio; take the
  // first one present rather than assume every site succeeded. Falls back to 12 (old behavior) if
  // no site has it (e.g. an all-stale-cache edge case).
  const periodDaysSample = sites.find((s) => s.trueRevenuePeriodDays)?.trueRevenuePeriodDays;
  const realRateAnnualizeFactor = periodDaysSample ? (365 / periodDaysSample) : 12;
  const totals = {
