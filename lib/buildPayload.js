...
function recordFor(code, name, c, full) {
  ...
  const rec = {
    ...
    enquiries: {
      ...
      // reservationConversions (Enquiry -> Reservation) — REBUILT 17 Jul 2026 (task #310) as the same
      // kind of plain PERIOD-RATIO as `conversions` above: reservation-stage rows this period (see
      // reportMap.js's reservation_stage_count) as the numerator, this period's total enquiries as the
      // denominator (page.js does the division, same as `conversions`).
      ...
      // close (low single digits up to ~10%), but a plain count ratio does (23.9% raw in our own June
      // data — in the same ballpark as legacy's 19.8%, with the remaining gap explained by our total
      // enquiry count already running ~10% below legacy's, a known/accepted discrepancy documented
      // above `enquiries:`). So legacy isn't tracking individual leads here at all — just dividing two
      // independent aggregates. Also fixes the live/in-progress month always looking artificially low:
      // a period-ratio needs no cross-month data at all, so there's no "next month doesn't exist yet"
      // gap to wait out.
      reservationConversions: lf.reservation_stage_count || 0,
...
export async function buildPayload(currentMonth, prevMonth) {
  ...
  for (let mi = 0; mi < months.length; mi++) {
    ...
    monthly[mk] = Object.keys(idx).filter(code => idx[code][mk] && idx[code][mk].occupancy && idx[code][mk].occupancy.total_units > 0)
      .map(code => {
        const rec = recordFor(code, nameOf[code] || code, idx[code][mk], false);
...
export async function buildPayloadRange(fromMonth, toMonth) {
  ...
  const sites = codes.map((code) => {
    const recs = rangeMonths.filter((mk) => idx[code][mk] && idx[code][mk].occupancy && idx[code][mk].occupancy.total_units > 0)
      .map((mk) => {
        const rec = recordFor(code, nameOf[code] || code, idx[code][mk], true);
        applyAutobillDailyAverage(rec, mk, autobillDailyMap);
        return rec;
      });
