...
export async function buildPayload(currentMonth, prevMonth) {
  ...
  const sites = Object.keys(NAMES)
    .map(code => {
      const rec = recordFor(code, nameOf[code] || NAMES[code] || code, (idx[code] && idx[code][cur]) || {}, true);
  ...
  for (const s of sites) {
    ...
    s.discountPlans = p.discountPlans || [];
    s.discountUnitsTotal = p.discountUnitsTotal || 0;
  }
...
export async function buildPayloadRange(fromMonth, toMonth) {
  ...
  const sites = codes.map((code) => {
    const recs = rangeMonths.map((mk) => {
        const rec = recordFor(code, nameOf[code] || NAMES[code] || code, (idx[code] && idx[code][mk]) || {}, true);
        applyAutobillDailyAverage(rec, mk, autobillDailyMap);
        return rec;
      });
