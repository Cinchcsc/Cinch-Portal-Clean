...
const inquiryChannelKey = (label) => String(label ?? '').trim().toLowerCase();
...
const visibleMarketingLeadBase = (enquiries) => {
  ...
  const fromChannels = Object.entries(enquiries.channels || {}).reduce((sum, [label, row]) => (
    isVisibleMarketingChannel(label) ? sum + (Number(row?.enquiries) || 0) : sum
  ), 0);
...
const visibleMarketingLeadConverted = (enquiries) => {
  if (!enquiries) return 0;
  const fromChannels = Object.entries(enquiries.channels || {}).reduce((sum, [label, row]) => (
    isVisibleMarketingChannel(label) ? sum + (Number(row?.converted) || 0) : sum
  ), 0);
  if (fromChannels) return fromChannels;
