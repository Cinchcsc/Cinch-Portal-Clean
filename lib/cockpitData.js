...
export async function readCockpitData() {
  ...
  const { data: rows, error } = await admin
    ...
    .select('site_code,snapshot_date,total_charge,total_payment,categories')
  ...
  const byDate = {};
  for (const r of rows || []) {
    const o = (byDate[r.snapshot_date] ??= { date: r.snapshot_date, total_charge: 0, total_payment: 0, sites: [] });
    o.total_charge += Number(r.total_charge) || 0;
    o.total_payment += Number(r.total_payment) || 0;
    o.sites.push({ code: r.site_code, total_charge: Number(r.total_charge) || 0, categories: r.categories || [] });
  }
