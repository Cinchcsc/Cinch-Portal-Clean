...
function hasFullDailyCoverage(curve, start, end) {
  if (!Array.isArray(curve) || !curve.length || !(start instanceof Date) || !(end instanceof Date)) return false;
  const seen = new Set(curve.map((row) => String(row?.date || '')));
  for (const day = new Date(start); day.getTime() <= end.getTime(); day.setDate(day.getDate() + 1)) {
    if (!seen.has(ymd(day))) return false;
  }
  return true;
}
...
function syncLatestCurrentMonthCockpitPoint(curve, portalPayload, currentMonthKey, currentDayKey) {
  ...
  if (existingIdx < 0) {
    const [year, month, day] = String(currentDayKey || '').split('-').map(Number);
    const priorDayKey = year && month && day ? ymd(new Date(year, month - 1, day - 1)) : null;
    const latestStoredDayKey = curve.reduce((latest, row) => {
      const dateKey = String(row?.date || '');
      return dateKey > latest ? dateKey : latest;
    }, '');
    // Continued audit hardening (11 Aug 2026): daily_financial_snapshot can occasionally miss JUST
    // the most recent complete day even though the portal current-month financial slice already has
    // the correct month-to-date totals for that day. In that one narrow case the old "replace only,
    // never append" rule left Cockpit lagging a day behind the rest of the portal. Still do NOT hide
    // broader cockpit gaps: only synthesize the latest point when the stored curve is otherwise
    // contiguous right up to the prior day, i.e. this is a one-day lag rather than a partial-history
    // problem.
    if (!priorDayKey || latestStoredDayKey !== priorDayKey) return { curve, synced: false };
    return {
      curve: [...curve, syncedPoint].sort((a, b) => String(a?.date || '').localeCompare(String(b?.date || ''))),
      synced: true,
    };
  }
...
export async function readLatestCockpitRefreshAt() {
...
export async function readCockpitData(monthKey) {
  ...
  if (isCurrentVisibleMonth) {
    try {
      ...
      const portal = await readPortalPayloadFreshCurrentMonthStable();
  ...
  // A cockpit month is only complete when every calendar day in its visible reporting window has a
  // stored point. Carry-forward padding is a display fallback for partial history, not evidence that
  // the missing day was ever actually pulled.
  const expectedCompleteEnd = isCurrentVisibleMonth
    ? lastCompleteDay(new Date())
    : new Date(target.getFullYear(), target.getMonth() + 1, 0);
  const complete = latestCurveDate != null && hasFullDailyCoverage(curve, monthStart, expectedCompleteEnd);
  ...
  return {
    ...
    // Keep generated_at on the cockpit route honest about the underlying cockpit snapshot material,
    // not the newer portal current-slice timestamp that may have been merged in-memory for just the
    // latest point. The page-level freshness label already considers the main portal payload
    // separately; inflating cockpit generated_at here makes the cockpit job look fresher than it is.
    generated_at: generatedAtMs ? new Date(generatedAtMs).toISOString() : null,
    synced_portal_generated_at: syncedPortalGeneratedAtMs ? new Date(syncedPortalGeneratedAtMs).toISOString() : null,
