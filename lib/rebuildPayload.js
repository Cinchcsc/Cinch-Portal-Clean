...
import { buildHistoryPoint, isPortalPayloadShapeUsable, mergeFreshCurrentMonth, payloadFromLiveCurrent, readPortalPayload, summarizeHistoricalMonthlyCoverage, writePortalPayload } from './portalPayload.js';
import { STALE_MS, checkPullLock, startPullLog, startPullLogLenient, finishPullLog, recordCompletedPullLog } from './pullLock.js';
...
export async function runRebuildPayload(opts = {}) {
  ...
  const storedBuildVersionMismatch = canUseFastRefresh && !storedBuildVersionCurrent;
  ...
  if (!skipLockCheck) {
    ...
    if (!unlocked.ok) {
      ...
      else await recordCompletedPullLog('rebuild', 'skipped', unlocked.message, startedAtIso);
  ...
  const finishOrBackfillLog = async (status, detail) => {
    ...
    return recordCompletedPullLog('rebuild', status, detail, startedAtIso);
  };
  ...
  try {
    ...
    const result = await withRetry(async () => {
      if (!forceHistoricalRepair && canUseFastRefresh && !historyAnalysis.requiresFullRebuild && !monthBoundaryFinalizationNeeded) {
        ...
        if (!storedBuildVersionMismatch && !requestedHistoricalMonths.length && hasOnlyOlderHistoricalRepairs && !opportunisticRepairMonths.length) {
      ...
      if (canUseFastRefresh && !historyAnalysis.requiresFullRebuild) {
        ...
        if (shouldRepairHistoryThisRun) {
          ...
          // 13 Aug 2026 (egress audit): only stamp the new build_version when every planned month
          // actually landed this run. The old check here only asked what we PLANNED to repair
          // (repairingAllHistoricalMonths), so a run that deferred even one chunk — transient DB
          // pressure, missing raw coverage — still marked the WHOLE historical payload as fully
          // repaired, permanently hiding those deferred month(s) from every future version-mismatch
          // pass (they'd never be retried again until the next manual version bump). That silently
          // defeats the one-time-repair mechanism this build_version marker exists for.
          const fullyRepairedThisRun = repairingAllHistoricalMonths && deferredRepairMonths.length === 0;
          payload = mergeHistoricalMonthlySlices(payload, repairedMonthly, {
            nextBuildVersion: fullyRepairedThisRun ? PORTAL_PAYLOAD_BUILD_VERSION : null,
          });
        }
        let liveCurrent;
        try {
      ...
      await writePortalPayload(payload, {
        generatedAt: new Date().toISOString(),
        updateExisting: !!stored?.payload,
      });
    ...
    await finishOrBackfillLog('ok', detail);
    return {
      status: 'ok',
