function transientFlags(message) {
  ...
  const isTransientSupabaseEdge = /\b(?:520|521|522|523|524|525|526)\b|cloudflare|schema cache|web server is down|unknown error|ssl handshake failed|invalid ssl certificate/i.test(message);
  ...
  const isAbortLike = /\babort(?:ed)?\b|the operation was aborted|this operation was aborted/i.test(message);
  return { isTimeout, isTransientSupabaseEdge, isTransientTransport, isAbortLike };
}

function shouldRetryMessage(message) {
  const { isTimeout, isTransientSupabaseEdge, isTransientTransport, isAbortLike } = transientFlags(message);
  return isTimeout || isTransientSupabaseEdge || isTransientTransport || isAbortLike;
}

export function isRetryableSupabaseMessage(message) {
  return shouldRetryMessage(String(message || ''));
}

function retryDelayMs(error, baseDelayMs) {
  const rawRetryAfter =
    Number(error?.retry_after) ||
    Number(error?.retryAfter) ||
    Number(error?.cause?.retry_after) ||
    Number(error?.cause?.retryAfter) ||
    0;
  if (!Number.isFinite(rawRetryAfter) || rawRetryAfter <= 0) return baseDelayMs;
  // Respect retry-after guidance enough to slow the hammering, but cap it so shared retry callers
  // still fit inside request/runtime budgets.
  return Math.max(baseDelayMs, Math.min(rawRetryAfter * 1000, 15000));
}
  ...
  for (let attempt = 1; attempt <= attempts; attempt++) {
    try {
      ...
      if (result && typeof result === 'object' && 'error' in result && result.error) {
        ...
        if (!shouldRetryMessage(message) || attempt === attempts) return result;
        await new Promise((resolve) => setTimeout(resolve, retryDelayMs(result.error, delayMs)));
        continue;
      }
      return result;
    } catch (error) {
      ...
      if (!shouldRetryMessage(message) || attempt === attempts) throw error;
      await new Promise((resolve) => setTimeout(resolve, retryDelayMs(error, delayMs)));
    }
  }
  throw lastError;
}
