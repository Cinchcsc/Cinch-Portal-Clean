...
export async function checkPullLock(options = {}) {
  ...
  let readErr = null;
  try {
    const result = await withRefreshLogTimeout(
      retryOnStatementTimeout(async () => admin
        .from('refresh_log')
  ...
  for (const row of runningRows || []) {
    ...
    try {
      const { error: markErr } = await withRefreshLogTimeout(
        retryOnStatementTimeout(async () => admin.from('refresh_log').update({
          status: 'timeout',
...
export async function startPullLog(kind) {
  ...
  const { data: logRow, error } = await withRefreshLogTimeout(
    retryOnStatementTimeout(async () => admin.from('refresh_log').insert({ status: 'running', kind, started_at: new Date().toISOString() }).select('id').single()),
    REFRESH_LOG_WRITE_TIMEOUT_MS,
    'refresh_log insert',
  );
...
export async function finishPullLog(logId, status, detail) {
  if (!logId) return;
  try {
    await withRefreshLogTimeout(
      retryOnStatementTimeout(async () => admin.from('refresh_log').update({ finished_at: new Date().toISOString(), status, detail: detail ?? null }).eq('id', logId)),
      REFRESH_LOG_WRITE_TIMEOUT_MS,
