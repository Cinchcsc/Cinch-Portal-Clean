...
// lib/pullCockpit.js (kind: 'cockpit'), and lib/rebuildPayload.js (kind: 'rebuild').
...
// triggered one. So this is ONE shared lock across all four kinds, not four independent ones — all
...
// EXPORTED 28 Jul 2026: lib/rebuildPayload.js's production-hardening pass needs to compute how much
// longer a currently-running lock has left before it self-clears (so a rebuild waiting on the lock
// can poll at a sensible interval instead of a fixed blind one), which means this threshold can no
// longer stay private to this file.
...
export async function checkPullLock() {
  ...
  const { data: runningRows, error: readErr } = await admin.from('refresh_log').select('id,kind,status,started_at').eq('status', 'running').order('started_at', { ascending: false });
