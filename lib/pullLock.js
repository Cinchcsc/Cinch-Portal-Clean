...
export async function checkPullLock(options = {}) {
  ...
  const { data: runningRows, error: readErr } = await retryOnStatementTimeout(async () => admin
    .from('refresh_log')
  ...
  for (const row of runningRows || []) {
    ...
    const { error: markErr } = await retryOnStatementTimeout(async () => admin.from('refresh_log').update({
      status: 'timeout',
...
export async function startPullLog(kind) {
  ...
  const { data: logRow, error } = await retryOnStatementTimeout(async () => admin.from('refresh_log').insert({ status: 'running', kind, started_at: new Date().toISOString() }).select('id').single());
...
export async function finishPullLog(logId, status, detail) {
  if (!logId) return;
  try {
    await retryOnStatementTimeout(async () => admin.from('refresh_log').update({ finished_at: new Date().toISOString(), status, detail: detail ?? null }).eq('id', logId));
  } catch (error) {
