...
import { checkPullLock, startPullLogLenient, finishPullLog, recordCompletedPullLog } from './pullLock.js';
...
async function runRetriedQuery(fn, attempts = 5, delayMs = 2000) {
  return withTransportRetry(async () => {
    const result = await retryOnStatementTimeout(fn);
...
export async function runPull({ reports, concurrency = Number(process.env.SITELINK_PULL_CONCURRENCY) || 1, rebuildPayload = true, shard, shards, triggerLabel = null } = {}) {
  ...
  try {
    ...
    await runRetriedQuery(async () => admin.from('sites').upsert(allLocations.map(code => ({ code, name: code })), { onConflict: 'code', ignoreDuplicates: true }));
  ...
  // this would start truncating for real.
    ...
    let lastPrevId = 0;
    for (;;) {
      const { data } = await runRetriedQuery(async () => admin.from('raw_report').select('id,site_code,report').eq('month', prevMonthKey).gt('id', lastPrevId).order('id').limit(PREV_PAGE));
  ...
  // works no matter which reports this particular invocation pulled, and is robust to any future
    ...
    const { data: micRows } = await runRetriedQuery(async () => admin.from('raw_report').select('site_code,data').eq('month', curMonthKey).eq('report', 'move_ins_outs'));
    const { data: rrRows } = await runRetriedQuery(async () => admin.from('raw_report').select('site_code,data').eq('month', curMonthKey).eq('report', 'rent_roll'));
    ...
    async function pullLoc(loc) {
      for (const key of reportKeys) {                                  // reports sequential within a location
        ...
        for (const month of ms) {
          ...
          await runRetriedQuery(async () => admin.from('raw_report').upsert(
    ...
    // day's row instead of creating duplicates.
      ...
      if (curMoveInsOuts && curRentRoll) {
        ...
        try {
          await runRetriedQuery(async () => admin.from('autobill_daily').upsert(
          ...
          errors.push(`${loc}/autobill_daily: ${describeError(error)}`);
        }
      }
    }
