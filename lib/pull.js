...
import { resetSiteLinkClients } from './sitelink.js';
import { checkPullLock, startPullLogLenient, finishPullLog } from './pullLock.js';
import { describeError } from './describeError.js';
import { normalizePortalPayloadForStorage } from './portalPayload.js';
...
const isRetryableSiteLinkPullError = (error) => {
  const message = String(error?.message || error || '');
  const code = Number(error?.retCode);
  return (
    code === -90 ||
    code === -99 ||
    /server busy|general exception|getaddrinfo|enotfound|econnreset|etimedout|socket hang up|fetch failed|timed out|service unavailable|http error 503|ssl handshake failed|web server is down/i.test(message)
  );
};
...
// billing_frequency ADDED 22 Jul 2026 (task #308, R6's authoritative Rate formula): pulled alongside
// rent_roll for the previous complete month too, purely for consistency — both reports jointly feed
// the same billing-adjusted Rate/ssRate calc in buildPayload.js's recordFor(), so they should always
// be available for the same set of months (avoids one being present without the other for any month).
const TWO_MONTH = new Set(['occupancy', 'management', 'lead_funnel', 'move_ins_outs', 'insurance_activity', 'merchandise', 'rent_roll', 'true_revenue', 'rental_activity', 'financial', 'insurance_roll', 'discounts', 'billing_frequency']);
...
// step is ever reached). Raising plan tier would not solve that shared-budget problem by itself, so
...
export async function runPull({ reports, concurrency = Number(process.env.SITELINK_PULL_CONCURRENCY) || 1, rebuildPayload = true, shard, shards, triggerLabel = null } = {}) {
  const started = Date.now();
  const logId = await startPullLogLenient('pull', 'pull');
  const lock = await checkPullLock({ activeKinds: ['pull', 'cockpit', 'floor'], claimingLogId: logId });
  if (lock.locked) {
    console.error('[pull] ' + lock.message);
    await finishPullLog(logId, 'skipped', lock.message);
    return { status: 'skipped', message: lock.message };
  }
  try {

    const allLocations = (process.env.SITELINK_LOCATIONS || '').split(',').map(s => s.trim()).filter(Boolean);
    if (!allLocations.length) throw new Error('SITELINK_LOCATIONS not set');
    const { error: sErr } = await retryOnStatementTimeout(async () => admin.from('sites').upsert(allLocations.map(code => ({ code, name: code })), { onConflict: 'code', ignoreDuplicates: true }));
    if (sErr) throw new Error('sites seed failed (run `npm run init:sites` to see why): ' + sErr.message);

  // SITE SHARDING (ADDED 20 Jul 2026, task #327 follow-up): giving true_revenue its own solo cron
  // hour (removing rental_activity/discounts from the same invocation) turned out NOT to be enough —
  // a live manual trigger of true_revenue ALONE (29 sequential CustomReportByReportID calls) still
  // hit Vercel's hard 300s maxDuration kill (confirmed via FUNCTION_INVOCATION_TIMEOUT in the response
  // headers), proving the bottleneck is true_revenue's own per-call cost, not competition from other
  // reports sharing the batch. Splitting REPORTS further doesn't help once a batch is already down to
  // one report; splitting SITES across multiple cron hours does — same "give it its own budget" idea
  // applied one level down. `shards` divides `allLocations` by index (not by site code, so this
  // doesn't need SITELINK_LOCATIONS' exact contents known ahead of time — it just works on however
  // many sites are actually configured) and `shard` (0-based) picks this invocation's slice. See
  // vercel.json for true_revenue's 4-way split across hours 5/6/7/8.
    const locations = (shards && shards > 1) ? allLocations.filter((_, i) => i % shards === shard) : allLocations;

    const now = new Date();
    const lastComplete = lastCompleteDay(now);
    const startOfReportingToday = dayOnly(addDays(lastComplete, 1));
  // FIXED 23 Jul 2026 (Michael: "the portal should always reflect as of the last complete day"):
  // the current month should still be month-to-date, but NEVER through a half-finished day. These
  // pulls run in the morning, so using the literal current timestamp made July's live month include a
  // partial 23 Jul slice that would then silently grow later in the day. Cap the in-progress month at
  // the START of today instead, so a pull on 24 Jul reflects activity through 23 Jul only — the last
  // fully completed calendar day. The previous month still drives MoM deltas and stays available in
  // the selector.
  // Use the month containing the LAST COMPLETE DAY as "current" everywhere in this pull too. On the
  // morning of 1 Aug, for example, the latest complete day is still 31 Jul, so the live pipeline
  // should keep writing/bucketing July as the active month until 2 Aug's run.
    const months = [reportingCurrentMonthStart(now), reportingPreviousMonthStart(now)];
    const currentReportingMonthKey = ymd(months[0]);
    const reportKeys = (reports || DEFAULT_REPORTS).filter(k => REPORTS[k]);

    const errors = []; let ok = 0;
    const parseEndOf = (month) => {
      const end = endOf(month);
      return new Date(end.getTime() - 1);
    };
    const endOf = (month) => {
      const isCurrentReportingMonth = ymd(month) === currentReportingMonthKey;
      if (isCurrentReportingMonth) return startOfReportingToday;
    // FIXED 23 Jul 2026 (production-readiness audit): SiteLink's dated report end bound is
    // exclusive of the calendar day it falls on. For a closed month, the old
    // `new Date(y, m + 1, 0)` landed at midnight on the LAST day of that month, silently dropping
    // the entire final day's activity from every dated report pulled through this pipeline
    // (confirmed by the month-end probes for InquiryTracking and True Revenue). Closed months must
    ...
    async function tryPull(key, loc, month) {
      ...
      for (let attempt = 1; ; attempt++) {
        try {
          return await pullReport(key, loc, month, endOf(month), { parseEndDate: parseEndOf(month) });
        } catch (e) {
          if (attempt >= backoff.length || !isRetryableSiteLinkPullError(e)) throw e;
          resetSiteLinkClients();
  ...
  // this would start truncating for real.
    ...
    let lastPrevId = 0;
    for (;;) {
      const { data, error } = await retryOnStatementTimeout(async () => admin.from('raw_report').select('id,site_code,report').eq('month', prevMonthKey).gt('id', lastPrevId).order('id').limit(PREV_PAGE));
      if (error) throw new Error(error.message);
      existingPrev.push(...(data || []));
      if (!data || data.length < PREV_PAGE) break;
      lastPrevId = data[data.length - 1].id;
    }
  ...
  // re-splitting of the cron schedule.
    ...
    const { data: micRows, error: micErr } = await retryOnStatementTimeout(async () => admin.from('raw_report').select('site_code,data').eq('month', curMonthKey).eq('report', 'move_ins_outs'));
    if (micErr) throw new Error(`autobill dependency read failed (move_ins_outs ${curMonthKey.slice(0, 7)}): ${micErr.message}`);
    const { data: rrRows, error: rrErr } = await retryOnStatementTimeout(async () => admin.from('raw_report').select('site_code,data').eq('month', curMonthKey).eq('report', 'rent_roll'));
    if (rrErr) throw new Error(`autobill dependency read failed (rent_roll ${curMonthKey.slice(0, 7)}): ${rrErr.message}`);
    ...
    async function pullLoc(loc) {
      for (const key of reportKeys) {                                  // reports sequential within a location
        ...
        for (const month of ms) {
          ...
          const { error } = await retryOnStatementTimeout(async () => admin.from('raw_report').upsert(
    ...
    // day's row instead of creating duplicates.
      ...
      if (curMoveInsOuts && curRentRoll) {
        ...
        const sampleDate = formatLocalYmd(lastComplete);   // represented business date, not pull timestamp day
        const { error: adErr } = await retryOnStatementTimeout(async () => admin.from('autobill_daily').upsert(
    ...
    if (rebuildPayload) {
      try {
        ...
        const { error: ppErr } = await retryOnStatementTimeout(async () => admin.from('portal_payload').upsert({ id: 1, generated_at: new Date().toISOString(), payload: normalizePortalPayloadForStorage(payload) }));
    ...
    const triggerDetail = [
      ...
      `reports=${reportKeys.join(',')}`,
      (Number.isFinite(shards) && shards > 1) ? `shard=${Number(shard) || 0}/${shards}` : null,
    ].filter(Boolean).join(' ');
    const detail = [
      triggerDetail || null,
      payloadError && ('payload: ' + payloadError),
      !rebuildPayload && 'payload rebuild skipped by design (rebuildPayload:false) — see /api/rebuild-payload',
      ...errors,
