...
export async function runPull({ reports, concurrency = Number(process.env.SITELINK_PULL_CONCURRENCY) || 1 } = {}) {
  ...
  async function pullLoc(loc) {
    let curMoveInsOuts = null, curRentRoll = null;                   // captured below for the autobill_daily sample, see after this loop
    for (const key of reportKeys) {                                  // reports sequential within a location
      ...
      for (const month of ms) {
        ...
        try {
          ...
          ok++;
          if (!isPrev && key === 'move_ins_outs') curMoveInsOuts = data;   // CURRENT month only — see below
          if (!isPrev && key === 'rent_roll') curRentRoll = data;
    ...
    // calendar day (while the month is still current — a closed/locked month no longer reaches this
    // point since curMoveInsOuts/curRentRoll above only capture !isPrev pulls) and average them once
    // the month closes — see lib/buildPayload.js's applyAutobillDailyAverage(). Upserts on
    // (site_code,month,sample_date) so re-running the pull twice in one day doesn't create two samples
    // for that day.
