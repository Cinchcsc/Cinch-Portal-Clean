...
import { formatLocalYmd, reportingCurrentMonthStart, reportingPreviousMonthStart } from './reportingPeriod.js';
...
export async function runPull({ reports, concurrency = Number(process.env.SITELINK_PULL_CONCURRENCY) || 1, rebuildPayload = true, shard, shards } = {}) {
  ...
  const { error: sErr } = await admin.from('sites').upsert(allLocations.map(code => ({ code, name: code })), { onConflict: 'code', ignoreDuplicates: true });
  ...
  // vercel.json for true_revenue's 4-way split across hours 5/10/11/12.
  ...
  const endOf = (month) => {
    ...
    if (isCurrentMonth) return startOfToday;
  ...
  async function tryPull(key, loc, month) {
    ...
    for (let attempt = 1; ; attempt++) {
      try { return await pullReport(key, loc, month, endOf(month), { parseEndDate: parseEndOf(month) }); }
  ...
  async function pullLoc(loc) {
    ...
    if (curMoveInsOuts && curRentRoll) {
      ...
      const sampleDate = formatLocalYmd(new Date());   // today, YYYY-MM-DD in local calendar time
