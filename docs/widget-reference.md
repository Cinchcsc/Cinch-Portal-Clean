...
| Reservations vs Move-outs | Stat card | ReservationList, ScheduledMoveOuts, RentRoll | dCancelled, dNeeded, QTCancellationTypeID, TenantID | Reservations = active/not-cancelled/future/not-already-tenant; Move-outs = ScheduledMoveOuts; Net = difference | current | **1,420 reservations, 285 move-outs, net +1,135** — still ~3x over the ~446 target, unresolved (Task #25) |
